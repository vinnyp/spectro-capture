# Research Results v2: Browsing a Collection at Scale

> **Provenance & how to read this.** A full re-run of [the collection-mode brief](browsing-a-collection-at-scale-research-brief.md), answering all 54 of its questions. Run 2026-08-24 by three parallel research agents with live web search, each required to label every source DOCUMENTED / INFERRED and, where original, **MEASURED**; two of the three produced primary measurement — one benchmarked SQLite and CIEDE2000 on the hardware named below, one read the macOS SDK headers and the live sandbox profile — and orchestrator-verified claims are flagged inline. **This v2 supersedes [the first-pass results](browsing-a-collection-at-scale-research-results.md) on any point where the two conflict**, because the first run answered only 28 of the 54 questions and its gap list was not trustworthy: 14 of its 26 claimed gaps cited the research agent's own retrieval limits rather than an actual absence in the literature. **Role in this repo:** research, not decisions — nothing below is settled until an ADR says so ([AGENTS.md §2](../../AGENTS.md#2-read-before-changing-anything)), and the collection-mode design it explores is still queued behind the Data Foundation and Collection Mode PRDs ([docs/product/README.md](../product/README.md)).

**Measurement environment for MEASURED figures:** Apple M5 Max, 128 GB, macOS 26.6 (25G72). System SQLite **3.51.0**. Synthetic corpus: 100,000 rows, three text columns, three `REAL` L\*a\*b\* columns, a 144-byte 36-band spectral BLOB (29.3 MB base table). Single-threaded, warm cache, median of 5–20 runs.
...
**For a colour catalogue this cuts both ways, which is the finding:** a swatch is the highest-discriminability cue available (grid's best case) *and* the row is spec-heavy — L\*a\*b\*, ΔE, supplier, batch (table's best case). **That is the strongest available argument that this app needs both a swatch grid and a table**, not a grid with a list bolted on.
...
3. **Over-broad `@Observable` invalidation** — the WWDC25 headline; see [the acquisition v2 doc](acquisition-experience-research-results-v2.md) §9.
...
**SQLite's R\*Tree supports 1–5 dimensions** (3-D Lab needs 7 columns) and its documentation is explicit that it is a prefilter: "An R\*Tree index does not normally provide the exact answer but merely reduces the set of potential answers from millions to dozens." It also warns that simultaneous reads and writes to the same R-tree can fail with `SQLITE_LOCKED`.
