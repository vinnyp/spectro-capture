...
## PR #17 agent-build amendment and review response (2026-09-17)

**Scope:** [PR #17](https://github.com/vinnyp/spectro-capture/pull/17), from `main` at `b26b663`; initial audit commits `01b009a` / `ec0d741`, followed by this response to the [review](https://github.com/vinnyp/spectro-capture/pull/17#pullrequestreview-5244162836) and [eleven owner decisions](https://github.com/vinnyp/spectro-capture/pull/17#issuecomment-5725041707). This appends to the historical lock record; peer review of the amendment remains required, and aligned requirements do not claim implementation.

**Initial amendment:** agent-focused measurement, regeneration, damage, file-action, deletion and fixture tables; F33 archive/history isolation and F34 switching policy; SQLite polling correction; consolidated acceptance scenarios and fixtures. Review restored the general salvage promise and read-only floor, established interim move/re-read and export behavior, fixed quarantine/restore/fixture semantics, and restored Status/Commit PR tracking.

**Owner decisions:** DF F36–F46 map one-to-one to the owner's numbered decisions; F33/F34 stand with F34's dated halted-capture confirmation. DE F14 transcribes the archive/state export contract settled by DF F37. Historical F7/OQ 10 wording is preserved with dated clarification, as is F30's map-relocation history.

**Re-derived inventory:** DF — 46 primary requirements, 13 surface rows, 51 other lettered rows (initial amendment: 45), 8 metrics, 28 copy states, F1–F46, 17 active OQ entries (12 open, 5 answered). DE — 13 primary requirements, R4.4a surface, 1 metric, 4 copy states, F1–F14, 4 OQ entries (2 open, 2 answered). Retired IDs remain reserved; existing IDs and section anchors remain intact.

**Mechanical validation:** DF body 7,995 / 8,000 words; DE 3,972 / 4,000, using the lock method (strip comments, code fences, link targets and table pipes, then count whitespace tokens). Active product/root references: 2,031 local links/anchors checked, no broken targets; no malformed tables or new duplicate IDs; `git diff --check` clean. Documentation-only: fixture definitions and acceptance criteria are specified, not implemented or run against an app; build/hardware evidence remains unchecked in post-lock.

### Inline dispositions

| Review comment | Response |
| :--- | :--- |
| [4043724712](https://github.com/vinnyp/spectro-capture/pull/17#discussion_r4043724712) | Restored the general salvage promise under owner decision 1 / F36. Removed the publication precondition from the requirement and post-lock entry; R5.5d/R7.3 still require complete readable data, read-write output and named resolutions. ADR-0003 owns per-invariant mechanics without narrowing E5. |
| [4043724715](https://github.com/vinnyp/spectro-capture/pull/17#discussion_r4043724715) | R2.3a explicitly records initial with no correction question after current-value quarantine; R2.3b now excludes items with no current value. R2.9/R2.4 agree, and DJ3 asserts that recovery raises neither E11 nor E26. |
| [4043724717](https://github.com/vinnyp/spectro-capture/pull/17#discussion_r4043724717) | Added R3.3g under F42: unreadable authoritative measurements retain sets/stamps and are listed separately as excluded by damage. R7.1/R7.5 and the completion oracle exclude this class, so retained quarantine no longer prevents completion. |
| [4043724720](https://github.com/vinnyp/spectro-capture/pull/17#discussion_r4043724720) | Applied owner decision 6 / F41. R7.7/R7.7e use a hand-authored live-kind snapshot for sc_simulated=false and mock-provided simulated snapshots for true; no Device Management field or identity rule changed. |
| [4043724724](https://github.com/vinnyp/spectro-capture/pull/17#discussion_r4043724724) | R1.2 again defines SQLITE_READER_FLOOR and promises readability with the app closed. Quarantine, sample archive-unavailable and unreadable-derived-set marks are explicitly plain-SQLite data, included in R7.1 read-back and DJ5 queries. |
| [4043724729](https://github.com/vinnyp/spectro-capture/pull/17#discussion_r4043724729) | Promoted reason/mark ownership into R2.3h and A → B → C behavior into R2.3i. R7.2 now exercises R2.3a–i, including incoming reasons and answering without creating a reading or changing either timestamp. |
| [4043724733](https://github.com/vinnyp/spectro-capture/pull/17#discussion_r4043724733) | Applied owner decision 5 / F40 in R2.3f, R7.7g and DJ2. Restore owns copies of samples/decoded values and fresh current-version six-space derivations under the collection reference, respecting R3.5, while leaving the source reading and its sets unchanged. |
| [4043724736](https://github.com/vinnyp/spectro-capture/pull/17#discussion_r4043724736) | Added R5.5f for unreadable persisted derived sets, induced by R7.2/R7.3 and fixture R7.7i. E32/R7.3l offer a requested rebuild at the current algorithm version (no version bump), retain the damaged set and preserve canonical selection/reading marks. Both new marks and recovery are observable; archive export semantics now live in R5.5a and the outbound obligation under F37. |
| [4043724741](https://github.com/vinnyp/spectro-capture/pull/17#discussion_r4043724741) | Applied owner decision 2 as DF F37 and DE F14. DE R1.1 now emits empty unavailable-archive payload cells while canonical export runs, without new columns/states; R1.1/R1.3 make quarantined override superseded. Removed the free-standing prohibition and synchronized both obligation directions, status and README; OQ 21 now concerns a future distinct mark/version bump. |
| [4043724742](https://github.com/vinnyp/spectro-capture/pull/17#discussion_r4043724742) | Applied owner decision 3 / F38: move/re-read are disabled during active, paused or halted capture and enabled after interruption. R1.5/R1.9 and R7.3c/j define the rule; E9/E15/E25/E27 explain it and route to the existing End session flow. OQ 19 concerns only future re-enablement. |
| [4043724745](https://github.com/vinnyp/spectro-capture/pull/17#discussion_r4043724745) | Applied owner decision 4 / F39. R5.3 guarantees the item list, canonical values and history marks for E1/E16/E23/E24, with R7.3/DJ3 verification. Restored E1’s look-at-it and E23’s work-read-only wording; OQ 18 covers extras while existing export restrictions/salvage rules remain. |
| [4043724746](https://github.com/vinnyp/spectro-capture/pull/17#discussion_r4043724746) | Applied owner decision 9 / F44: R7.3b checks target version/integrity/ownership before closing the original, and failed checks leave the original open. R1.3 cites Capture R3.2/R3.5; DJ3 and the verification sentence agree, with no no-file-open state. |
| [4043724749](https://github.com/vinnyp/spectro-capture/pull/17#discussion_r4043724749) | Recorded F45 and dated owner confirmation under F34; E22’s halted variant names End session. Restored OQ 10’s original attributed answer and added dated F35 clarifications there and under F7/F30. Fixed the F2/F3/F10/F12/F14/F32 mappings to the relocated sibling amendment map. |
| [4043724752](https://github.com/vinnyp/spectro-capture/pull/17#discussion_r4043724752) | Applied owner decision 8 / F43. R7.7m provides a fabricated prior-format fixture and a test-only supported reader/migration, exercising E2/E3/E23/E24 from v1 without altering the production compatibility floor. M5 names it; R7.7 names Export R4.1’s header-only golden, and R7.7i covers current damage with and without readable history. |
| [4043724758](https://github.com/vinnyp/spectro-capture/pull/17#discussion_r4043724758) | Addressed all six: R7.2 separates missing derivation stamps from missing-condition absence; R7.1 includes illuminant/observer; §7 indexes embedded tests; R7.3f asserts an unchanged file without selecting an updater; R7.5 resumes without re-deriving healthy sets already at the target version/reference (reference matters for same-version setting changes); Capture R4.13 maps to R7.4; R7.6h names R5.5a–c/f. |
| [4043724760](https://github.com/vinnyp/spectro-capture/pull/17#discussion_r4043724760) | Applied owner decision 11 / F46: restored per-row Status and the Commit PR name, retaining the lettered contract tables and common v1 declaration. |
| [4043724764](https://github.com/vinnyp/spectro-capture/pull/17#discussion_r4043724764) | The main journeys index is now the sole row inventory; companion Exercises lines are removed. The header explicitly adds no rules, blind re-scan cites Capture R8.7, and the unsupported file-selection/no-open-file outcome has been replaced by F44’s original-file-preserved behavior. |
| [4043724767](https://github.com/vinnyp/spectro-capture/pull/17#discussion_r4043724767) | E4 again names unaffected collection usability and the readable-earlier-reading count; R7.6h asserts them. Conditional markers prefix sentences/actions, including current/history and floor; E14 now matches E8’s deletion wording. E1/E23 restore the requested Cataloger wording under F39. |
| [4043724771](https://github.com/vinnyp/spectro-capture/pull/17#discussion_r4043724771) | Restored the active Capture outbound obligation, pointed the inbound Export fixture reference at the matrix, linked OQ 17–21 Feeds to their owning sections/documents, and distinguished same-connection polling from cross-process push notifications in the evidence paragraph. |
| [4043724777](https://github.com/vinnyp/spectro-capture/pull/17#discussion_r4043724777) | README rows preserve the shared review log and Import F59 history while naming PR #17’s DF F33–F46 and DE F14 amendments. Removed DE’s machine-local governance path, used PR #17 wording in post-lock, and appended a dated review-response record with re-derived counts to the shared review log. |


## PR #17 round-2 review response (2026-09-17)

**Scope:** delta from reviewed commit `649025e`; [round-2 review](https://github.com/vinnyp/spectro-capture/pull/17#pullrequestreview-5244347644) and [three owner decisions](https://github.com/vinnyp/spectro-capture/pull/17#issuecomment-5725692929). Round 1 was confirmed closed by the reviewer; this response addresses all ten new inline comments, pending peer verification.

**Decisions:** F47 makes derived-set recovery automatic and retires E32/R7.3l; F48 restores E14’s deletion statement while preserving E8’s never-scanned wording; F49 records proposed M10 only in post-lock. R5.5d again names `correction-unconfirmed`; action routing, F35/DE F14 maps, P1/floor markers and journey indexes are synchronized.

**Current inventory:** DF — 46 primary requirements, 13 surface rows, 50 other lettered rows, 8 metrics, 27 copy states, F1–F49, 17 active questions (12 open, 5 answered). DE — 13 primary requirements, R4.4a surface, 1 metric, 4 copy states, F1–F14, 4 questions (2 open, 2 answered). E32/R7.3l are explicitly retired and never reused; M10 is a post-lock proposal, not a metric row.

**Validation:** DF 7,987 / 8,000 words and DE 3,972 / 4,000 using the established lock method. Local links/anchors: 2,040 checked, none broken; valid Markdown tables; fence maps complete and inside their map sections; copy states owned by surfaces and requirements; all other reviewed IDs preserved. Documentation-only: acceptance criteria are specified, not application/hardware test results; pending build evidence remains unchecked.

| Review comment | Response |
| :--- | :--- |
| [4043886847](https://github.com/vinnyp/spectro-capture/pull/17#discussion_r4043886847) | Applied owner round-2 decision 1 as F47. R5.5f now automatically regenerates unreadable derived sets at the current version without a bump, retaining the damaged set as superseded and preserving canonical selection/reading marks. R3.3a names this case; R7.7i/DJ3 assert its result. E32 and R7.3l are removed and their IDs retired; no export or completion exception was introduced. |
| [4043886851](https://github.com/vinnyp/spectro-capture/pull/17#discussion_r4043886851) | Moved the two-current-readings example into R5.5d and restored the exact correction-unconfirmed enum: later-recorded stays current, the other is retained as correction-unconfirmed (R2.2/R2.4). DJ3 uses the same value; the general salvage promise is unchanged. |
| [4043886853](https://github.com/vinnyp/spectro-capture/pull/17#discussion_r4043886853) | Moved DE F14 directly after F13 inside the fence-to-row table, before Rejected findings. Removed R4.2 from its mapping because that row does not itself carry the new value-level contract. |
| [4043886859](https://github.com/vinnyp/spectro-capture/pull/17#discussion_r4043886859) | Extended F35’s current ranges to R2.3a–i, R3.3a–g and R7.7a–m, covering R2.3h/i. R7.3 ends at k because owner decision 1 retires l; that retirement is explicit in F35 and the retired-ID map. F33 now maps R5.5f, with F47 supplying its automatic-regeneration decision. |
| [4043886864](https://github.com/vinnyp/spectro-capture/pull/17#discussion_r4043886864) | R7.6e now asserts every action, which are disabled in flight, and the End session route. E9 remains explicitly covered by R7.6b’s Copy IDs; the test no longer assumes only two enabled ways forward. |
| [4043886867](https://github.com/vinnyp/spectro-capture/pull/17#discussion_r4043886867) | R7.3c/j now link both Capture’s ordinary End session path and Device R5.11’s path from a halt, preserving its save-failure warning. The copy Variants note agrees; R1.3 now distinguishes Capture R3.5’s in-flight trio from R3.2’s status enum. |
| [4043886876](https://github.com/vinnyp/spectro-capture/pull/17#discussion_r4043886876) | Marked DJ2’s no-display clause as P1 while keeping its Capture R8.7 ownership citation. The journey header now states that marked clauses run only once their owning P1 rows land. |
...
## PR #18 peer-review response — 2026-09-18

**Reviewed head:** `b198da1`. All 17 inline findings addressed; the [owner decision comment](https://github.com/vinnyp/spectro-capture/pull/18#issuecomment-5732380642) resolves all twelve decision items. Eleven new fences F19–F29 correspond to decisions 1 and 3–12; decision 2 restores standing F14/DF F37 and explicitly requires no new fence. F15–F18 now distinguish the approved audit scope from those detailed decisions. The earlier audit record above describes the initial submission, not the final reviewed behavior.

**Material outcomes:** quarantined canonical/history rows retain readable provenance and available archives; Device and DF mirror unknown simulation cells; DF adds empty collection R7.7n and comparison/payload fixtures; allocation uses Import’s comparator; exactly three mixed-case sRGB names are exempt; non-spectral mismatch is derivable without a CSV mark; preview variants and version tests have explicit owners. No architecture, hardware result, mid-session or overwrite policy is selected.

| Inline finding | Disposition |
| :--- | :--- |
| [4048163394](https://github.com/vinnyp/spectro-capture/pull/18#discussion_r4048163394) | Applied owner decision 1 as F19. R1.2 defines empty as unknown; Device R6.5 and its Export obligation mirror true/false from known snapshots and empty otherwise. DF R7.7e now asserts all three values, using R7.7h for the empty case; post-lock records the sibling amendment. |
| [4048163397](https://github.com/vinnyp/spectro-capture/pull/18#discussion_r4048163397) | Restored the archive-only boundary under standing F14/DF F37, per owner decision 2, without adding a new payload fence. R1.1i and new canonical-quarantine R1.1s empty colour/spectral/qualifier cells but defer payloads to R1.1d/l. E1 and EJ1 agree; dated notes reconcile the historical F10/DF F29 clauses. |
| [4048163402](https://github.com/vinnyp/spectro-capture/pull/18#discussion_r4048163402) | Applied owner decision 3 as F20. DF R7.7n is the empty-collection fixture; R4.1h cites it, R4.2 enumerates it, and both obligation directions and the post-lock amendment name it. |
| [4048163407](https://github.com/vinnyp/spectro-capture/pull/18#discussion_r4048163407) | Applied owner decision 4 as F21. R1.1h now covers only never-scanned rows; new R1.1s retains readable quarantined-current snapshot/simulation/basis/measured-at without a sequence column. History retains the same provenance plus its history fields; E1 and EJ1 use the same distinction. |
| [4048163412](https://github.com/vinnyp/spectro-capture/pull/18#discussion_r4048163412) | Applied owner decision 5 as F22. F17 expressly supersedes the old import-file-order clause. R2.4a owns persisted-order allocation and all three examples, citing Import R2.5/R2.6; R2.4c states that another column’s collision can rename a literal import_ name while stored names/values remain unchanged. |
| [4048163419](https://github.com/vinnyp/spectro-capture/pull/18#discussion_r4048163419) | Applied owner decision 6 as F23. Both R2.4b predicates use Import R2.3, while emitted names preserve stored spelling with prefixes prepended. DF R7.7a and EJ1 cover case/whitespace variants. R2.4a states both export kinds allocate identically because the whole sc_ namespace is reserved. |
| [4048163425](https://github.com/vinnyp/spectro-capture/pull/18#discussion_r4048163425) | Applied owner decision 7 as F24. R2.4 enumerates exactly sc_sRGB_gamut_clipped, sc_sRGB_source_space and sc_sRGB_rendering_intent; every other sRGB column uses lower snake_case. |
| [4048163431](https://github.com/vinnyp/spectro-capture/pull/18#discussion_r4048163431) | Applied owner decision 8 as F25. R2.1 now excepts non-spectral original-reference qualifiers. R1.1k owns missing-condition precedence and states that mismatch is derivable from qualifiers and the chosen-condition column, with no separate exported mark; EJ1 asserts it. |
| [4048163439](https://github.com/vinnyp/spectro-capture/pull/18#discussion_r4048163439) | R1.1n now names newer/below-floor files and cites DF R5.5d/R2.2 specifically for file-level failures. Its result owns the override, preserves per-reading/archive eligibility, and leaves automatic derived-set recovery with DF; the resolving prose was removed. |
| [4048163445](https://github.com/vinnyp/spectro-capture/pull/18#discussion_r4048163445) | Applied owner decision 9 as F26. DF R7.7i includes unused slots and a sample with no instrument-supplied payload; R4.2 and EJ1 assert all three absence cases. E1b uses swatches for canonical quarantine and E1c uses readings for history. |
| [4048163453](https://github.com/vinnyp/spectro-capture/pull/18#discussion_r4048163453) | Added the paired R2.5 append-versus-value-form-change scenario, including other bump triggers. R4.1a lists R2.5’s help-doc obligations; R4.2 explicitly names R1.2/R2.1, DF R7.7a–n and the generated corpus, and routes refused states to R4.3 with no golden while eligible migrated states use goldens. |
| [4048163463](https://github.com/vinnyp/spectro-capture/pull/18#discussion_r4048163463) | Applied owner decision 11 as F28. R1.1o owns pre-write recomputation and export/app-version disclosures; E1 renders version placeholders and the calculation-version qualifier. E1b/c suppress row count when equal to item count, as R1.1o requires. |
| [4048163470](https://github.com/vinnyp/spectro-capture/pull/18#discussion_r4048163470) | Applied owner decision 12 as F29: restored the shared priority hedge and status definitions. Traceability now includes lettered requirement subdivisions and copy variants. |
| [4048163475](https://github.com/vinnyp/spectro-capture/pull/18#discussion_r4048163475) | Restored the browsing-v2 §10 evidence pointer in the fence companion’s Phase 0 inventory. |
| [4048163483](https://github.com/vinnyp/spectro-capture/pull/18#discussion_r4048163483) | The inbound Capture obligation and Capture’s outbound Export obligation now name R1.1g beside R2.3; DF R7.7l agrees. R7.7n is named in the fixture obligations. The Device pointer distinguishes its simulated concept from the emitted sc_simulated header and mirrors the new nullable rule. |
| [4048163487](https://github.com/vinnyp/spectro-capture/pull/18#discussion_r4048163487) | Split the missing-data and preview anchors; assigned E1a–g and listed them on R4.4a, inheriting E1 status/phase rules. Restored numeric constant rendering. Quarantined provenance is specified in R1.1i/s, not in free-standing copy prose. |
| [4048163494](https://github.com/vinnyp/spectro-capture/pull/18#discussion_r4048163494) | F15–F18 now retain the approved audit scope and point to the detailed, individually sourced owner decisions F19–F29, each with Why and a fence-to-row mapping. Payload restoration uses existing F14 as instructed. F17 expressly supersedes the old clause; removed the premature DE completion checkbox, kept build evidence unchecked, and added the authorized sibling-amendment records for PR #18. |

**Validation:** 1,979 active-document local links/anchors, zero broken; IDs/priorities/implementation tracking preserved against the reviewed head; 29/29 Export fences mapped, all new fences source the owner comment; 13 primary Export requirements, 30 added lettered rules plus the original surface; four copy states plus E1a–g; M1, E2–E4 and OQs unchanged. Export 3,585/4,000 words; DF 7,976/8,000 (lock count: remove HTML comments and fenced code, remove Markdown link destinations, replace table pipes with spaces, then split the whole PRD on whitespace). No new table-width defects; Capture’s pre-existing table widths are unchanged. Collision examples and case/whitespace variants traced against the stated comparison algorithm, without claiming platform-verification evidence. `git diff --check` and gitleaks pass; hooks remain enabled. Documentation only; application/hardware testing does not apply and first-build evidence remains unchecked.

## PR #18 round-two response — 2026-09-18

**Reviewed head:** `1476ffa`. Round one is closed by the peer review. All ten new inline findings are addressed; [owner decision 13](https://github.com/vinnyp/spectro-capture/pull/18#issuecomment-5732919975) becomes F30, requiring original vendor payload strings in CSV regardless of storage compression, with R1.1d/R4.1a and the DF outbound mirror carrying it.

| Inline finding | Disposition |
| :--- | :--- |
| [4048427031](https://github.com/vinnyp/spectro-capture/pull/18#discussion_r4048427031) | Restored all matrix defaults: both export kinds unless scoped, preserved R1.1b common fields including chosen condition, empty-field semantics, and the full per-value qualifier-cell definition. Removed the matrix-wide empty rule from R1.1i. |
| [4048427038](https://github.com/vinnyp/spectro-capture/pull/18#discussion_r4048427038) | Restored “never appear without” in R2.1 and explicitly separated R1.1b’s chosen-condition column from per-value qualifiers. |
| [4048427040](https://github.com/vinnyp/spectro-capture/pull/18#discussion_r4048427040) | R2.4b now starts from stored spelling and spacing, unnormalised; comparisons use Import R2.3, each prefix is followed by a recheck, and allocation occurs only after both predicates are false. |
| [4048427046](https://github.com/vinnyp/spectro-capture/pull/18#discussion_r4048427046) | Restored the explicit DF R2.1/R2.2/R2.4/R2.9 list, history inclusion and F31 citation, and linked the Rows cell. No unrelated DF requirements are imported by a broad range. |
| [4048427051](https://github.com/vinnyp/spectro-capture/pull/18#discussion_r4048427051) | Retitled R1.1i “Quarantined reading, history export” so it includes quarantined current rows in history output; removed the redundant no-sequence-column clause from R1.1s. |
| [4048427054](https://github.com/vinnyp/spectro-capture/pull/18#discussion_r4048427054) | Changed inbound Device provenance to F19, added R1.1g to outbound Capture, removed E1b/c from F19’s map, pointed refusal checks directly at R1.1n, and added R1.1o–r to R4.4a’s Req-IDs. |
| [4048427061](https://github.com/vinnyp/spectro-capture/pull/18#discussion_r4048427061) | Restored second-condition sets per reading in DF R7.7. DF’s status, README row 4 and Device R6.5’s Commit PR cell now record the PR #18 amendments. |
| [4048427071](https://github.com/vinnyp/spectro-capture/pull/18#discussion_r4048427071) | Pinned DF R7.7n to a newly created empty collection with no imported columns; EJ1 uses that exact composition. No additional emptied-after-import fixture was introduced. |
| [4048427075](https://github.com/vinnyp/spectro-capture/pull/18#discussion_r4048427075) | E1 now discloses the original reference for non-spectral readings, backed by R1.1q. E1d explicitly omits the per-data-row version sentence when there are no rows while retaining the version disclosure. |
| [4048427079](https://github.com/vinnyp/spectro-capture/pull/18#discussion_r4048427079) | R4.1a now says unnamed app headers/app-column order; R4.1b stores literal __APP_VERSION__ in golden data cells, validates actual versions before substitution, and preserves full-byte repeated-export checks. R4.1h/EJ1 use selected kind, EJ1 cites R4.2 for refusal exclusion, R1.1c scopes current measured-at to canonical, the review log states the counting method beside the prior count, and F15 notes no format has yet been released. Owner decision 13 is separately recorded as F30: R1.1d/R4.1a and DF’s outbound obligation require the original decompressed vendor string, byte-identical after CSV unquoting. |

**Validation:** 1,985 active-document local links/anchors, zero broken; all reviewed IDs, Export priorities/tracking, M1, E2–E4 and OQs preserved; 30/30 Export fences mapped; no new table-width defects (pre-existing Capture table widths unchanged). Export 3,683/4,000 words; DF 7,969/8,000, counting the whole PRD after removing HTML comments/fenced code and Markdown link destinations, replacing table pipes with spaces, then splitting on whitespace. Collision examples still trace under the stated comparator. `git diff --check` and gitleaks pass; hooks enabled. Documentation only: no application/hardware test evidence claimed; first-build checks remain pending.
