...
- **Post-lock list (consolidated from rounds 1–10 and the two lock-check sweeps):**
  - Sibling amendments already recorded as outbound lines: Capture Mode (its Data Foundation obligation row gains R4.5 and R1.10; R1.9 and R4.24 name Data Export too; E29's correction default; OQ 19 closing on F2/F14; OQ 24 re-opening R6.5's inventory), Device Management (UJ1/UJ1.2's location step; R1.21 naming Data Export; the hardware spike's scope gaining the wavelength grid, the payload round-trip, the analytics recipient and the reference set — **this must land before the spike is dispatched**, plan PL7-4), Inventory Import (R2.2 keeps positions and values, appends new columns), Vision and the product README ("raw payload canonical" is the stored mean).
  - Build-review items: a CSV → store → CSV round-trip case for a passthrough value; a golden asserting `sc_simulated` false; R7.6b listing the versions each file-opening state names; R7.2's oracle naming a cleared imported value; R7.6k asserting the delete counts on an item with no earlier readings; E8's never-scanned rendering; DE E1 at an empty collection; the collision tie-break for a literal `import_`-prefixed passthrough column.
  - ADR-0003 items: the generated ROWS_CEILING corpus's determinism contract (M6, M8 and DE R4.1 read from it); the regeneration job's progress record; the per-invariant salvage rules.
  - Documentation items: the help-docs column dictionary carries a formula-injection note for spreadsheet consumers, and nothing yet gates a release on that dictionary existing; the dogfood build's participant-facing onboarding says the interaction record is kept and a delete does not reach it; a redacted bug-report artifact; the remembered-mapping lifecycle; the serial on the device authorization path; the README's gamut gloss ("which most screens can't show accurately").
  - Drift watch: DF R7.7's fixture list and DE's outbound mirror of it are two copies of one list; whether a Collection Mode rename moves an imported column's stored name; OQ 2's ADR-0006 gloss.
  - v2 candidates recorded as deliberate omissions: field selection at export (serial, measured-at); no collection-identifier column; no mid-session export rule; no destination-overwrite rule.
- **Judgment calls recorded for the owner to overrule** (all stood at lock): the salvage conflict resolution (later-recorded reading stays current); reflectance as a fraction of 1; times in UTC with `Z`; a path in both volume classes shows the network state; the reflectance precision fixed when the golden is re-cut; E28–E30 lose the "way back" clause; the ceiling corpus is generated rather than checked in; DE R2.4 returns to fence F13's "exactly as stored" with the storage obligation on DF R1.2; prose trims to fit the budgets (rounds 9 and 10).
