...
- [x] **LP-1** — import `prd-inventory-import.md`, Inherited obligations, the "Capture Mode" line: append "; an import is refused while a session on that collection is in flight, and the routes [E40] offers resolve to the capture PRD's §7 paths — an active or paused session ended from there follows [the capture PRD's R7.5](../capture-mode/prd-capture-mode.md#7-pause-end-interruption-and-resume)". _Done — the Capture Mode obligations line now reads through a semicolon and carries the E40 routes plus the cross-PRD R7.5 link; the refusal clause is stated once._
- [x] **LP-2** — capture `prd-capture-mode.md:426`, the "Inventory import" line: Rows cell adds `[R6.1](#6-queue-navigation-and-reordering)`; Obligation cell appends "; and the design scale an import holds up under — ROWS_TARGET within IMPORT_BUDGET, ROWS_CEILING warned about and imported anyway ([R3.12](#3-the-capture-session), OQ 13)". _Done — R6.1 added to the Rows cell in numeric order, and the obligation cell names the scale R3.12 imposes (ROWS_TARGET within IMPORT_BUDGET, ROWS_CEILING warned about and imported anyway, OQ 13)._
- [x] **LP-3** — import fences `:5`: append "The capture file's text is canonical; an amendment lands there and is re-copied here." Capture fences F49 Decision: after "metrics M7 and M8, open questions 12 and 13" insert "(M8 and OQ 13 stay — clarification (1))". _Done — the import fence preamble names the capture text canonical, and F49's Decision carries the parenthetical "(M8 and OQ 13 stay — clarification (1))" with the history unrewritten._
- [x] **LP-4** — import `:189`: the label reads `[the capture PRD's M8](…)` with the possessive removed from outside the link. Import Legend "Status of this document": every row 🤝 Aligned; the F49 verification rounds (25, 26) are recorded in the capture review log; the document is locked with the capture PRD. _Done — the M8 possessive moved inside the link, and the Legend status paragraph now reads every row 🤝 Aligned, rounds 25-26 recorded in the capture review log, locked with the capture PRD._
- [x] **LP-5** — Flip import R4.1 and R4.2 → 🤝 Aligned (unanimous ALIGN, rounds 26). `docs/product/README.md` row 3: **Locked** — split from the capture PRD under F49 and verified in rounds 25–26; owner sign-off pending, no PR yet. _Done — R4.1 and R4.2 flipped to bare 🤝 Aligned; README row 3 now reads **Locked** — split from the capture PRD under F49 and verified in rounds 25-26; owner sign-off pending, no PR yet._
- [x] **LP-6** — checks: links and anchors in all eleven files; `grep -o '\[\(R\|E\|M\)[0-9.]*\](\.\./' docs/product/*/prd-*.md` returns nothing; no `&amp;`; two-sentence rule; six mermaid blocks; counts. _Done — 11 files, 823 links and anchors checked, 3 misses, all the device PRD's pre-existing `&amp;` headings; widened grep returns only the closed split-pass record; 0 `&amp;` in the ten capture and import files; 136 + 16 requirement rows, 0 over two sentences; six mermaid blocks parse OK; capture 146 🤝 / 0 ⌛️, import 17 🤝 / 0 ⌛️, copy 31 and 13._

## Lock record — both PRDs after the F49 split (2026-09-08)

- **Capture Mode:** 146 R and M rows 🤝 (136 requirement rows, 10 metrics; R8.1a–k and R11.15a–g are cells of their lead rows); copy 31 of 31 🤝. Retired under F46 and F49, never reused: R4.25, R7.4, R7.6, R7.10, R8.11, R10.2, R11.1, R11.2, R11.4, R11.9; R2.1–R2.14, R11.15h, M7; E4–E14, E38, E40 (moved to the import PRD).
- **Inventory Import:** 17 rows 🤝 (R1.1–R1.4, R2.1–R2.4, R3.1–R3.6, R4.1, R4.2, M1); copy 13 of 13 🤝; every row arrived locked from capture except R4.2, the carried observability obligation, verified in round 26. Fences F4 and F11 copied with the capture text canonical; F49 and clarification (1) in the capture fence file.
- **Rounds 25–26** verified the split (three lenses each, all diffing against the pre-split tree): no rule changed; the two findings above Minor were the dropped observability rule (restored as R4.2) and bare cross-PRD IDs (labels now name the owning PRD, both Legends state the rule). Round-26 residue was applied as an editorial lock pass (LP-1–LP-6 above), checked by grep and link resolution.
- **Checks at lock:** 823 links and anchors across the eleven files, the only misses the device PRD's literal `&amp;` headings; no bare cross-PRD row label; no `&amp;` in the capture and import files; every requirement row at most two sentences; six mermaid blocks render; no placeholders or template comments.
- **Process files** (23 round fix files, the compaction map and pass, the split pass, the round-25 fix file) were resume points whose every box is ticked and whose findings this log records; they are deleted at the owner's instruction and are recoverable from the branch history up to this commit.
- **Both PRD headers keep `Status: draft`** for the owner to flip. No PR until the owner says so.
...
- **AM-I2** — the import PRD's outbound obligations table gains a Data Export line: imported and identity columns carried into the export in the export PRD's R2.3 order, a collision renamed rather than dropped (its R2.4) → DE `:182`.

Verification: a bounded staff-engineer check (Claude/Opus, 2026-09-16) over the amended rows — five of six transcribed their authority exactly; E29's first landing stated the default on the state's own check-or-correct axis, the word fence DF F12 bars, and dropped F3's next-review occasion (a Blocker) — rewritten on the Data Foundation PRD's supersession-reason axis with both occasions; the Data Export obligation on the import PRD (DE `:182`) landed as AM-I2 on the same check's observation. Noted for the next Data Foundation pass: its "which row here carries…" paragraph (DF `:236`) does not yet name the capture line's R1.10 and R4.5; and E29 arrives with the QC & Comparison PRD, so until then the Data Foundation PRD's E11 is the surface for the default. Safe to merge.
