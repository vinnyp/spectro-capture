...
**Priority.** Build order within v1, not a cut line. P0 is the first phase — the file, canonical value and version history, derived values, migration and compatibility, and the verifiability rows other PRDs lean on; P1 the second — export, deletion, privacy. A recommendation, not a decision.

Provisional constants: each is named, carries a candidate and its OQ id, and none ships in a release with its OQ open — a dogfood build not being a release for that rule. An unqualified "OQ n" or row ID is this document's. DERIVATION_VERSION is a stamp rather than a provisional constant and carries no OQ; ROWS_CEILING is the capture PRD's, under its OQ 13.
...
| R1.3 | v1 | P0 | Whether a user may keep several files, and whether more than one may be open at once, is OQ 1; until it closes there is one file, one open at a time ([the capture PRD's OQ 19](../capture-mode/prd-capture-mode.md#open-questions)). | ⌛️ Ready for Alignment |  |
...
| R2.6 | v1 | P0 | Version history is kept for HISTORY_RETENTION (candidate: for good, nothing aged out — OQ 4) and the file stays within STORE_SIZE_BUDGET (candidate ≤ 25 MB at 1,000 items × 10 readings, against a measured 20.53 MB compressed and 41.06 MB raw — OQ 5). Whatever those become, the app can enumerate what history holds and show what it costs. | ⌛️ Ready for Alignment |  |
...
| ID | Release | Pri | Requirement | Status | Commit PR |
| :--- | :--- | :--- | :--- | :--- | :--- |
| R4.1 | v1 | P1 | A whole collection or a single item exports to CSV carrying each item's identity, every column its import brought in, the wavelength and reflectance data, and all six derived spaces ([R3.1](#3-derived-values-and-gamut-honesty), [U6](../vision.md#use-cases)); it reads and never writes. Every export names its own format version and the app version that wrote it, that version changing only when a column's meaning does. | ⌛️ Ready for Alignment |  |
| R4.2 | v1 | P1 | The sRGB and HSL columns never appear without the columns qualifying them — source space, rendering intent, gamut-clipped — beside the illuminant and observer, so a consumer taking the bare triple does so knowingly. What that clipping column is called is OQ 8. | ⌛️ Ready for Alignment |  |
| R4.3 | v1 | P1 | Export emits a `simulated` column, true or false, from the acquiring device's snapshot ([the device PRD's R6.5](../device-management/prd-device-management.md#6-mock-device-layer), [R7.6](#7-verifiability)). | ⌛️ Ready for Alignment |  |
| R4.4 | v1 | P1 | An export carries current canonical values only unless the user asks for version history, and the surface says which before it runs ([E6](prd-data-foundation-copy.md#error--state-copy)). Whether history export ships in v1, and in what shape, is OQ 9. | ⌛️ Ready for Alignment |  |
| R4.5 | v1 | P1 | An export that cannot finish — no room, no permission, the destination gone — leaves no partial file and says what happened ([E7](prd-data-foundation-copy.md#error--state-copy)). A test makes the destination fail mid-write and observes nothing left behind. | ⌛️ Ready for Alignment |  |
...
| R6.3 | v1 | P1 | A delete is undoable for DELETE_UNDO_WINDOW (candidate: until the app quits — OQ 10), the undo restoring the item with its history intact rather than as a fresh empty row. | ⌛️ Ready for Alignment |  |
...
| Inventory Import | The one matching rule for Swatch Codes and collection names, and an import that lands whole or not at all ([its R2.3](../import/prd-inventory-import.md#2-target-mapping-and-the-matching-rule), [its R3.2](../import/prd-inventory-import.md#3-preview-and-commit)) | [R1.1](#1-the-file-the-user-owns), [R2.2](#2-canonical-value-and-version-history) |
...
| Telemetry, help docs | The vendor SDK's own usage analytics disclosed in plain language, separately from any telemetry the app itself sends | [R6.5](#6-deletion-and-privacy) |
| Capture Mode | Where the correction-against-re-measurement question is asked, if at capture time, and how it sits beside that document's check-it-or-correct-it choice (OQ 3) | [R2.4](#2-canonical-value-and-version-history) |
...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| 1 | How many files may a user's work live across, and may more than one be open? | None. One file holds the collections ([the capture PRD's F1](../capture-mode/prd-capture-mode-fences.md)); the count was handed here. | One file, one open at a time. | Owner, after ADR-0003. | [R1.1](#1-the-file-the-user-owns), [R1.3](#1-the-file-the-user-owns) | open |
...
| 3 | Is the correction-against-re-measurement question asked, and where? | None. It cannot be inferred from the data, so a silent default would fabricate it; overlaps the capture PRD's E29. | Ask once, outside the heads-down loop; a re-scan taken during capture defaults to correction. | Owner; amends the capture PRD if the ask lands there. | [R2.4](#2-canonical-value-and-version-history) | open |
| 4 | HISTORY_RETENTION | None. Retrofitting retention onto a store that cannot enumerate its history is the named trap. | Candidate: for good, nothing aged out. | Owner, against measured dogfood growth, after OQ 5. | [R2.6](#2-canonical-value-and-version-history) | open |
...
| 7 | GAMUT_REFERENCE_SPACE and GAMUT_RENDERING_INTENT | None. "In gamut" is intent-relative and the platform never says which intent its check uses. | Candidates sRGB and relative colorimetric. | The half-day harness the research names: a Lab sweep along the sRGB boundary through each intent — then owner. | [R3.4](#3-derived-values-and-gamut-honesty) | open |
...
| 9 | Does an export carry version history, and in what shape? | None. | Canonical values only; history an explicit choice, whether it ships in v1 being the question. | Owner, on what a dogfood data consumer asks for. | [R4.4](#4-export) | open |
| 10 | DELETE_UNDO_WINDOW | None. A reversible bulk delete is cheap. | Candidate: until the app quits. | Owner. | [R6.3](#6-deletion-and-privacy) | open |
...
| 12 | The vendor SDK's analytics, and whose document discloses them | None. The SDK sends events on connect, scan, and calibration, no colour data, and says the app must disclose it (per SDK docs). | The app discloses it; which document owns the wording is the question. | Observe the outbound traffic on hardware, then place the obligation — hardware. | [R6.5](#6-deletion-and-privacy) | open |
