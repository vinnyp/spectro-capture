...
Status: locked (2026-09-16); PR #17 agent-build amendment approved 2026-09-17 (F33–F46). Requirements remain aligned, not implemented.
...
| :--- | :--- | :--- | :--- |
| DJ2 | Fix a bad scan by measuring it again | R2.1–R2.9, R3.1, R7.1/R7.2 | [DJ2](prd-data-foundation-journeys.md#dj2-fix-a-bad-scan-by-measuring-it-again) |
| DJ3 | Open a file made by an older or a newer app | R1.3/R1.5/R1.9, R2.9, R5.1–R5.8, R7.3 | [DJ3](prd-data-foundation-journeys.md#dj3-open-a-file-made-by-an-older-or-a-newer-app) |
| DJ4 | Delete a swatch | R6.1–R6.5, R7.2, R7.6k/l | [DJ4](prd-data-foundation-journeys.md#dj4-delete-a-swatch) |
| DJ5 | Query the file without the app | R1.1–R1.6, R2.1–R2.9, R3.1–R3.5, R5.6, R7.1 | [DJ5](prd-data-foundation-journeys.md#dj5-query-the-file-without-the-app) |
...
**Retired row IDs.** Never reuse them; the [F30 migration map](prd-data-foundation-fences.md#retired-id-map) is in the fences companion.
...
| R7.6g | Item detail, data lines | The conditions, derivation version, every mark, the reading count | R2.4, R3.2, R3.5, R5.5 | E4, E11, E31, E32 | 🤝 Aligned |
| R7.6h | Damaged reading or archive | The affected reading/sample/set, variant, current value, readable-history count, other items usable, applicable recovery actions | R2.9, R5.5a–c/f | E4, E31, E32 | 🤝 Aligned |
...
| R1.3 | P0 | One file is active (F2), with [Import R2.3](../import/prd-inventory-import.md#2-target-mapping-and-the-matching-rule)'s matching rule applied everywhere: codes unique per collection, collection names unique across the file ([Capture R1.2](../capture-mode/prd-capture-mode.md#1-collections)). Refuse switching during active/paused/halted capture (E22, F34/F45); otherwise precheck the target before closing the original, preserving interrupted sessions ([R7.3b](#file-actions), F44; session states: [Capture R3.2/R3.5](../capture-mode/prd-capture-mode.md#3-the-capture-session)). | 🤝 Aligned |  |
| R1.4 | P0 | The app writes only at the user's chosen location and sends none of the file's contents elsewhere; location-managed sync is R1.7's exception. Inject [Device R6.12](../device-management/prd-device-management.md#6-mock-device-layer) reachability and enumerate R6.17 outbound attempts: Demo permits only enabled telemetry, live adds vendor authorization/SDK analytics (R6.6); observed requests must be a subset of that configuration's permitted recipients and carry no item, reading or payload. | 🤝 Aligned |  |
...
| :--- | :--- | :--- |
| R3.3a | Derivation algorithm changes | Bump DERIVATION_VERSION and regenerate every eligible persisted condition set (R3.3g excludes quarantined readings), including non-chosen sets and non-spectral sets at their original fixed reference; retain prior sets as superseded, with one live set per reading/condition |
...
| R5.4 | P0 | The app checks the file on open within INTEGRITY_CHECK_BUDGET (candidate ≤ 1 s at ROWS_CEILING ([the capture PRD's OQ 13](../capture-mode/prd-capture-mode.md#open-questions)) — this document's OQ 13, [M8](#success-metrics)), reserving the slower, fuller check for a user's request or a failed quick one, and never repairs the user's file unasked. | 🤝 Aligned |  |
| R5.5 | P0 | Handle file-level damage, isolated measurement damage, archive-only damage and failed upgrades according to [Damage classification](#damage-classification), with the source never repaired unasked (fence F33). Salvage writes readable data to a user-chosen fresh file, retains it until the user removes it, resolves source invariants as specified there, and follows [File actions](#file-actions) for completion and failures. | 🤝 Aligned |  |
...
| R5.5f | Persisted derived set unreadable; measurement intact | Mark set unreadable (E32); on request regenerate from intact measurement under R3.3 at current version without bumping it, retaining the damaged set; canonical selection and reading marks unchanged |

For two current readings, salvage selects the later-recorded one and retains the other, with an unresolved correction relationship between the two visible under R2.4; no reading is dropped. Per-invariant rules, including equal record times, belong to ADR-0003 without narrowing the general salvage promise (F36).
...
| R7.3j | E15 Try again / Move my file; E25/E27 Move my file / Keep it here | Retry the unsaved operation after revalidation / run R1.9's move / acknowledge the location risks; move failure leaves the source at its original location, disabled during active/paused/halted capture, enabled after interruption; Go to the session routes to Capture’s End session flow (F38) |
...
| R7.3l | E32 Rebuild colour values / Not now | Regenerate under R5.5f / keep the set marked unreadable; no repair without request |
...
| R7.7 | P0 | Check in fixtures for every released format plus a fabricated prior version exercising upgrades from release one (R7.7m); simulated snapshots use [Device R6.9](../device-management/prd-device-management.md#6-mock-device-layer), but the non-simulated snapshot is hand-authored as live-kind in the fixture (F41). [R7.7a–m](#fixture-matrix) cover the set; generate ROWS_CEILING with a second condition's set per reading for M6/M8 and [Export R4.1](../export/prd-data-export.md#4-verifiability)'s header-only golden, with corpus determinism owned by ADR-0003 and export assertions by Export R4.2. | 🤝 Aligned |  |
...
| R7.7e | Mock-supplied simulated snapshot and hand-authored live-kind snapshot | Preserve simulation identity; Export R4.2 golden asserts both `sc_simulated` values without live hardware |
...
| R7.7i | Archive-only damage; damaged current with and without readable history; damaged history with healthy current; damaged derived set | R5.5a–c/f SQL-readable marks and current-selection outcomes; retained damage records; export empty archive cells and quarantined state per F37 |
