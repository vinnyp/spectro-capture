...
| R7.7 | P0 | Check in every released-format fixture plus fabricated prior-version R7.7m; simulated snapshots use [Device R6.9](../device-management/prd-device-management.md#6-mock-device-layer), live-kind snapshots are hand-authored (F41). [R7.7a–n](#fixture-matrix) own the inventory; generate ROWS_CEILING with second-condition sets per reading for M6/M8 and [Export R4.1](../export/prd-data-export.md#4-verifiability)’s header-only golden, ADR-0003 owning determinism and Export R4.2 assertions. | 🤝 Aligned |  |
