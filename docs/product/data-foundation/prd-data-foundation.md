...
| R7.6 | v1 | P0 | A test lists what each surface in [Surfaces](#surfaces) offers and shows, and exports a collection and asserts its column set — the qualifying columns beside every sRGB and HSL column, the `simulated` column, and the format version ([R4.1](#4-export)–[R4.3](#4-export)). | ⌛️ Ready for Alignment |  |
