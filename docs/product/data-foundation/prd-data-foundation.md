...
| R4.8 | v1 | P0 | Every column the app emits carries one reserved name prefix, fixed for the life of a format version, stated in the help docs, and closed to any column an import brought in. A colliding passthrough column is emitted under a stated altered name rather than dropped or overwritten, the export surface naming the change ([E6](prd-data-foundation-copy.md#error--state-copy)). | ⌛️ Ready for Alignment |  |
