...
Archive-only damage preserves the canonical value under [Data Foundation F33](../data-foundation/prd-data-foundation-fences.md#f33--isolate-archive-and-history-damage-from-current-value-loss). Export handling of that damaged archive is gated by [its OQ 21](../data-foundation/prd-data-foundation.md#open-questions); R1.1's empty-for-absent rule must not be silently applied to unreadable bytes.
