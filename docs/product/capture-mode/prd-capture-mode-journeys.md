...
Use Device's simulated seam plus Capture R11.5–R11.16 and the Data Foundation readback/fault controls. Declare every exercised seam input under Device R6.25; only noninstant default latency is exempt. A comparison against SAMPLE_TOLERANCE declares below/equal/above fixtures, chosen mode and reference, not values copied from the implementation under test. A display-reference variation uses a pair available in the declared illuminant/observer reference-pair set (Capture R11.6 / Device R6.9); it never asserts a live pair unsupported by OQ 21.
...
| T6 | P0; pending rows; first bulk start has fully passing readiness with no advisory; device state can change via R11.6 | Start bulk (passing gate hidden), end it with rows pending; change readiness to blocking and start bulk again | Both starts execute every check; second presents the block, starts no session, requests no measurement and leaves the queue unchanged from before that second start | R3.10/R7.5/R11.6; Device §4 |
...
| UJ1-e | Saved samples and spread; spectral / non-spectral fixtures | Configure two supported reference pairs in the declared reference-pair set (R11.6 / Device R6.9), then change display reference between them | No re-scan, no measurement or agreement-verdict mutation; spectral derivation follows selected reference; non-spectral stays at original reference with required mark | R1.5/R4.9/R4.12/R4.24/R11.6; Device R6.9; DF R3.3/R3.5; F67 |
...
| Case | Given | When | Assert | Rows |
| :--- | :--- | :--- | :--- | :--- |
| UJ3-a | Measuring build (R11.16); pending A/B; N=3; guard and agreement record-only; nonzero latency | Accept three triggers on A, then three on B | One measurement per accepted trigger; R11.7 records at least two distinct channels for every sample and row confirmation; each durable set precedes row confirmation and one advance; captured +2, pending −2, deferred unchanged; R11.16 records exactly N accepted trigger interactions and zero other accepted interactions on each clean row (M11) | R4.1–R4.8/R4.12–R4.15/R4.23/R11.3/R11.7/R11.16; M11 |
...
| UJ3-k | Virtual pacing clock plus independent real clock; observable cues/announcements | Run configured paced captures and induced failures | Record both clocks and cue kind/pre-emption; assert timing budgets and configured distinct cues; cues promised through two senses by R4.7/R4.10/R4.13/R5.3/R5.13/R5.14/R7.17 fire on at least two distinct channels; rejection fires on at least one non-visual channel (R4.3); every capture-state announcement is recorded as not requiring focus (R11.14), without judging OQ14’s policy; haptic exposure remains hardware-gated | R4.7/R4.8/R4.13/R4.17/R11.7/R11.14; M1/M10 |
...
| UJ3.1-l | No session open; enabled guard; N_CONSEC_HARD=2; two eligible set-aside rows | Start a one-row scan from look-through, Skip after a failed attempt / exhaust K in separate runs, then repeat on the other row and reopen the file | Gate runs and connected instrument binds; Skip/K-exhaustion leaves the row set aside with evidence and ends the one-row session; each session reads guard zero initially and recorded one on deferral, with no pause or carry-over; ended-session counters remain readable after reopen | R8.3/R3.11/R3.13/R5.9/R8.1a/R11.11; F58/F68 |
...
| UJ3.4-c | Single-session chain from an initially unmeasured collection; completes with/without deliberately-set-aside rows | Read E24 then Done | E24 shows collection-state captured/deferred/pending counts equal to this session’s own, R3.1 elapsed time excluding pauses/halts/review, and captured count divided by that time in hours; set-aside line iff count nonzero; Done dismisses summary only; collection entry points remain | R7.15/R7.19/R11.12/R11.15f–g |
...
| UJ3.5-g | Six-row queue; three linked sessions with own capture times 10,5,7 minutes and distinct newly captured rows 2,1,3; no flag/demotion/rework | Resume twice, complete the final row and read chain summary/records | R11.11 reads own times 10/5/7 min and captured outcomes 2/1/3, with chain time 22 min at the summary and 10/15 min as running sums of own times, not additional readback fields; final E24 shows collection counts captured 6/deferred 0/pending 0, 22 min and rate 6 ÷ (22/60) per hour, with R11.11 identifying resumed-chain time/rate; 7 or 47 min fails | R3.1/R7.15/R7.18/R11.11; M2; F57/F66 |
| UJ3.5-g2 | UJ3.5-g’s six-row chain; additionally defer row X in session 1, then capture X first among session 3’s three captures | Complete session 3 and read E24, collection surface and R11.11; separately end early immediately after X is captured | Completion tallies equal collection state: captured 6/deferred 0/pending 0, never historical deferred 1; E24 omits the set-aside line; in the branch ended just after capturing X, the collection surface and R11.11 show captured 4/deferred 0/pending 2, and E23 has no set-aside sentence; chain time/rate and own-session evidence remain distinguishable | R7.5/R7.15/R7.18/R10.3/R11.11/R11.12/R11.15g; F66 |
| UJ3.5-g3 | Collection already has 4 captured rows from an earlier ended session; new chain captures the 3 remaining pending rows across two linked sessions with own capture counts 1/2 and own times 5/10 min | Complete the chain; compare E24 headline, rate and R11.11; in a separate branch End after this chain’s first 2 captures at 10 min | E24 headline captured 7 with elapsed 15 min; rate 3 ÷ (15/60) = 12/hour, never 28/hour from collection count; R11.11 confirms chain basis; end-early branch has collection captured 6 but rate readback 2 ÷ (10/60) = 12/hour, never 36/hour | R7.5/R7.15/R11.11; M2; F69 |
