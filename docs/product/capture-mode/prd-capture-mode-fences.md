...
- **F1** no Library — R1.1. **F2** measurement-settings scope — R1.3, R1.5, R1.10, R4.6. **F3** rename and delete are Collection Mode's — R1.8. **F4** the import target is chosen in the app — moved to the import PRD (F49): its [R2.1](../import/prd-inventory-import.md#2-target-mapping-and-the-matching-rule). **F5** jump by code plus manual reordering — R6.7, R6.8, R6.11.
