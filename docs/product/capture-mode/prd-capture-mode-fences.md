...
**Decision:** The consecutive-failure guard has one counter, N_CONSEC_HARD (candidate 2, floor 2 — OQ 3), counting consecutive rows set aside by any of these routes (gloss amended round 22 to match R5.9): auto-deferred after K_FAILED_ATTEMPTS, "Skip" after a failed attempt, or a disagreeing set the operator set aside. N_CONSEC_FLAGGED is retired; F18, F37, and F40 read as one counter; OQ 3 tunes one number.
...
**Decision:** §2 (R2.1–R2.14), the thirteen copy states only §2 cites (E4–E14, E38, E40), journeys UJ 2, UJ 2.1 and UJ 2.2, metrics M7 and M8, open questions 12 and 13 (M8 and OQ 13 stay — clarification (1)), fences F4 and F11, and surface row R11.15h leave this PRD for `prd-inventory-import.md`, which takes the same three-file shape as this one (PRD, `-journeys.md`, `-copy.md`) plus its own `-fences.md` and `-oq-results.md`. Every PRD lives under a feature directory: `docs/product/capture-mode/`, `docs/product/import/`, `docs/product/device-management/`; `vision.md` and `README.md` stay at `docs/product/`. Retired here, never reused: R2.1–R2.14, R11.15h, E4–E14, E38, E40, M7 (listed in the Legend as F46's retired IDs are); the import PRD numbers its own rows from R1.1 and keeps the E numbers it inherits. Citations of a moved row from this PRD become links into the import PRD, and the Inherited-obligations table gains an "Inventory import" line. F4 and F11 are copied into the import fence file with provenance. No rule changes; every moved row keeps its 🤝 status with the note "moved under F49"; one verification round (staff, interface, test) over both documents, then both re-lock.
...
### F50 — Mirror the Device Management amendment (2026-09-18)

**Decision:** [Device F12](../device-management/prd-device-management-fences.md#f12--carry-captures-device-obligations-2026-09-18), approved with the Device audit recommendations, carries this PRD's settled OQ 16 notes. R11.3 and the obligation table now name Device R6.27's existing P0 latency; the §5 obligation specifies process-interruption resume as a new session while halt recovery continues the current session, and records Device R5.18's P0 priority. OQ 16 is answered with results; no hardware gate or new Capture behavior is decided. Peer review of the amendment remains pending.

**Clarified 2026-09-18 ([owner decisions](https://github.com/vinnyp/spectro-capture/pull/19#issuecomment-5734032720)):** Device F16 defines Device E33 confirmation followed by Capture E23's halted-session summary (including held-reading text, without Keep scanning); F24 leaves OQ 16 open pending Device re-lock. Under D11 / Device F25, F50 was moved under Fences after F49 today and added to the map; historical decision text is retained as history, not a current closure claim.
...
- **F50** Device amendment mirror — R7.5, R11.3, R11.12, E23 halted variant, Device obligation rows, OQ 16/results (pending Device re-lock).
