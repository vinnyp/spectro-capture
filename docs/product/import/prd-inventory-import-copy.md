...
Copy uses the Cataloger’s vocabulary, never SDK-speak. Every promise below is backed by a requirement row in the PRD.

Variants share a state ID; tests can distinguish the cause without matching text. Variant and scope labels below are documentation, not displayed text; render only the applicable body and actions. All offered actions below are observable under R4.1. Every listed state offers Cancel, including E44 after a failed commit; an in-progress commit finishes or rolls back under R3.2.
...
| E43 | Import preview | Import into ⟨collection⟩ | Target: ⟨collection⟩. Existing items: ⟨existing⟩. Encoding: ⟨encoding⟩. Separator: ⟨delimiter⟩. New: ⟨new⟩. Updated: ⟨updated⟩. Unchanged: ⟨unchanged⟩. Added columns: ⟨columns⟩. ⟨filled⟩ rows gain details they didn't have. Left out: ⟨excluded⟩ rows (listed with reasons). ⟨absent⟩ rows in the collection are not in this file — left untouched. Above-ceiling variant appends: This will bring the collection to ⟨n⟩ items, above the recommended limit of ⟨ceiling⟩. You can still import. | Import; Choose an encoding; Choose a separator; Cancel | 🤝 Aligned |
...
E43 renders the target and nonzero existing count as first-class lines, with encoding/delimiter controls always visible (R3.8n). Lists show names and source record numbers, omitting empty list lines and zero-count sentences under Capture §12; keep the target’s name visible independently of the existing-item count. `⟨n⟩` in its warning is resulting collection size; `⟨ceiling⟩` is ROWS_CEILING. Other count placeholders name the corresponding R3.6 count, with `⟨filled⟩` counting matched rows that gain absent fields under R3.6j; `⟨encoding⟩` and `⟨delimiter⟩` name the current read settings.
