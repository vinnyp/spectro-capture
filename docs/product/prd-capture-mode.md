...
3. Or I choose "Leave it set aside" with a note — it is missing, damaged, or not worth the time today ([copy, §12](#12-error--state-copy)) — or "Leave all set aside" with one note for everything that is left (fence F31).
...
The Commit PR column is where a row maps onto the work that lands it. Owner decisions F1–F33 are recorded in the [fence file](prd-capture-mode-fences.md); a row that implements a fence cites it, and no row re-argues one.
...
| Deferred-row review (the set-aside list) | Every set-aside row with its cause, how many attempts were made, and how many samples it kept | Leave one swatch set aside; leave every remaining swatch set aside; samples disagree; guard paused |
...
| R1.5 | v1 | P0 | Illuminant and observer are optional collection-level display defaults that arrive pre-filled at D50/2°, are never required at creation, and are editable at any time (fences F2, F28). Changing them never requires a re-scan, never alters a stored reading, and never changes an agreement verdict ([R4.9](#4-the-scan-loop), fence F28), because the raw measurement is what the app keeps and colour values are worked out from it ([AGENTS.md §8](../../AGENTS.md#8-vocabulary)). The pairs the user may choose from are the reference set the instrument's own toolkit offers — the full eighteen reference whites on Spectro-class hardware ([SDK audit](../briefs/nix-universal-sdk-audit-findings.md), per SDK docs; confirm on hardware — OQ 21); without the spectral entitlement only D50/2° is available, and the agreement check runs under D50/2° in either case (fence F28). | ⌛️ Ready for Alignment |  |
| R1.6 | v1 | P0 | The ISO 13655 measurement condition (M0/M1/M2) is a scan mode of the instrument, not an observer, and which scan modes a capture records is OQ 1 (Spectro 2 supports M0/M1 plus M2 on F2.x firmware — [SDK audit](../briefs/nix-universal-sdk-audit-findings.md), per SDK docs; confirm on hardware). Until OQ 1 closes the app keeps every mode a reading arrives with and uses M1 as the collection's scan mode ([R1.10](#1-collections)), so nothing is discarded while the question is open. | ⌛️ Ready for Alignment |  |
| R1.7 | v1 | P0 | A collection with nothing pending and nothing deferred shows the nothing-to-capture state ([E2](#12-error--state-copy)) in one of two variants: truly empty, offering import and add-item; or finished — every row captured — saying so and additionally offering a re-scan ([§8](#8-deferred-row-review-and-corrections)). A collection with nothing pending but deferred rows waiting is not this state at all: starting capture there opens straight into the set-aside list ([R3.9](#3-the-capture-session)). Nothing else happens. | ⌛️ Ready for Alignment |  |
...
| R2.14 | v1 | P0 | A mapped column present in the file but blank for a row is treated as a change, because it would clear the field, and appears under "updated" so the user sees it; a column omitted from the file entirely leaves existing values untouched. | ⌛️ Ready for Alignment |  |
...
| R4.9 | v1 | P0 | With two or more samples the app checks that they agree within SAMPLE_TOLERANCE — the largest ΔE2000 distance of any one sample from the set's mean ([R4.12](#4-the-scan-loop), fence F33), measured on the collection's chosen scan mode ([R1.10](#1-collections), OQ 1) and always computed under a fixed D50/2° reference whatever the collection's display defaults happen to be (fence F28). A distance exactly equal to SAMPLE_TOLERANCE agrees; only a larger one disagrees. The reference used, the statistic, and the spread are all recorded with the reading, so an old verdict can be read back and understood (candidate ΔE2000 2.0, a placeholder with no evidence behind the value; the statistic is a candidate too — OQ 3). With one sample the check is skipped (fence F10). No shipped precedent exists for this check; it is designed here, not inherited. | ⌛️ Ready for Alignment |  |
...
| R4.15 | v1 | P0 | The capture surface shows the current row's Swatch Code and Swatch Name; the position "row r of R", where r is the row's place in the full queue order and R is the number of rows in the queue, so the pair reads the same as the queue list does; the sample counter "sample n of N"; the tallies captured / deferred / pending, which are where the remaining work is read; and the recents strip of the last RECENTS_STRIP_LENGTH captured rows (a named provisional constant, candidate 5 — OQ 23) ([acq v2 §11 Q11.2](../briefs/acquisition-experience-research-results-v2.md#11-the-seam--capture-to-collection)). An insert grows R and moves r; a reorder moves both. Where the recents strip sits relative to the set-aside list is settled with the seam and is part of OQ 8 (fence F20). | ⌛️ Ready for Alignment |  |
| R4.16 | v1 | P0 | The capture surface carries no cancel and no abandon control, and "End session" sits deliberately away from anything that advances the queue — the Dynamics 365 precedent of removing the abandon affordance from the counting surface ([acq v2 §8 Q8.4](../briefs/acquisition-experience-research-results-v2.md#8-progress-orientation-and-session-completion)). A test can list what the capture surface offers and assert that no such control is among them ([R11.12](#11-demo-device-and-verifiability)). | ⌛️ Ready for Alignment |  |
| R4.17 | v1 | P0 | Nothing on the host is touched between the samples of one set and the operator never types metadata between scans ([acq v1 §1](../briefs/acquisition-experience-research-results.md)); the only typing in a bulk session is the operator's own Add item ([§9](#9-ad-hoc-capture-and-one-row-sessions), fence F13). An announcement policy for assistive technology in a roughly three-second loop is OQ 14, and the capture state is announced without requiring focus. A test can see every announcement the app made ([R11.14](#11-demo-device-and-verifiability)), which is what lets a policy be judged rather than described. | ⌛️ Ready for Alignment |  |
| R4.18 | v1 | P0 | A measurement already in flight when any queue action fires — Flag, Skip, Pause, a jump, Add item, "Re-take sample", or "Restart item" — belongs to the row that was current at the trigger press the app accepted. If that row no longer takes samples by the time the reading arrives, the reading is discarded and recorded as an attempt in that row's own history; it is never applied to another row (fence F32). A test can hold a reading open, fire each of those actions in turn, let the reading arrive, and read back that it landed on the row current at the press or went into that row's history — never onto the new current row ([R11.5](#11-demo-device-and-verifiability), [R11.11](#11-demo-device-and-verifiability)). | ⌛️ Ready for Alignment |  |
| R4.19 | v1 | P0 | "Re-take sample" discards the last sample of the current set and nothing else: the counter goes back by one, every earlier sample is kept, the row stays current and stays pending, and the next accepted trigger takes that sample again. | ⌛️ Ready for Alignment |  |
| R4.20 | v1 | P0 | "Restart item" discards every sample taken so far on the current item and returns it to sample 0. The row stays current and stays pending, the queue does not move, and no other row is touched. | ⌛️ Ready for Alignment |  |
| R4.21 | v1 | P0 | Either action pressed while a scan is in flight is rejected with the rejection cue of [R4.3](#4-the-scan-loop) and discards nothing. Neither is a failure: neither counts toward K_FAILED_ATTEMPTS nor toward either of the guard's counters ([§5](#5-per-scan-failure-and-the-consecutive-failure-guard)). Either pressed on a row that is already captured and confirmed answers with [E36](#12-error--state-copy), because the correction there is a re-scan ([§8](#8-deferred-row-review-and-corrections)). A test can take two of three samples, press "Re-take sample", and read back one sample kept, no attempt recorded, and the row still current ([R11.11](#11-demo-device-and-verifiability)). | ⌛️ Ready for Alignment |  |
...
| R5.7 | v1 | P0 | After any queue advance that was not a row-success confirmation — the moved-on cue of [R5.4](#5-per-scan-failure-and-the-consecutive-failure-guard), a Skip, or a Flag as missing — and before any trigger press on the new current row, Flag changes nothing: it says only that the row just left is already set aside and waiting ([E20](#12-error--state-copy)), so a reflexive "that one was wrong" never marks the next row missing (fence F16). There is nothing to demote in that window, because no row landed. | ⌛️ Ready for Alignment |  |
...
| R5.9 | v1 | P0 | The guard names two things: a hard failure is a per-scan failure the instrument itself reports, and a deferred row is any row set aside for review. N_CONSEC_HARD consecutive hard failures (candidate 2 — OQ 3), or N_CONSEC_FLAGGED consecutive rows deferred by the instrument (candidate 4 — OQ 3, counting a row deferred after K_FAILED_ATTEMPTS, by Skip after a failed attempt, or by a set that disagreed), pause the session with a caution through two senses naming the likely cause — placement, light, temperature, or the material itself — and offering "Check placement and resume" or "Recalibrate" ([E19](#12-error--state-copy); [device PRD §3](prd-device-management.md#3-calibration)). A row reaching captured resets both counters, and any sample the instrument accepts resets N_CONSEC_HARD, so an instrument that is working again clears its own record. The guard is modelled on Westgard multirules with clinical numbers that must be re-tuned for a hand-placed instrument ([acq v2 §4 Q4.2](../briefs/acquisition-experience-research-results-v2.md#4-mid-run-error-handling)). | ⌛️ Ready for Alignment |  |
| R5.10 | v1 | P0 | Rows the operator set aside by choice — Flag as missing or damaged, Skip mid-set with no failure, Flag on a row that just landed — never count toward either counter and never break a run, because the guard watches the instrument rather than the operator's decisions (fence F18). Lamp or calibration drift is not counted here either (fence F8). A test can flag five rows as missing in a row, observe no pause, and read back both counters untouched ([R11.11](#11-demo-device-and-verifiability)). | ⌛️ Ready for Alignment |  |
...
| R6.8 | v1 | P1 | The operator reorders before a session from the collection, or during a session from a queue list opened by a keyboard queue-management action (fence F5), either by dragging a pending row to a new position or by sorting on any metadata column — Swatch Code, Swatch Name, the alternates, or any other imported column. (Inherited obligation for the Collection Mode PRD: the sort and drag controls on the collection view; this PRD owns what a reorder does to the queue — fence F22.) | ⌛️ Ready for Alignment |  |
| R6.9 | v1 | P1 | Sorts respect the user's language and read the numbers inside a code as numbers, so "A2" comes before "A10" and not after it; a code with no number sorts by its letters, so "A" comes before "A2"; and rows that tie keep the order they were already in, so sorting a hand-made order twice never shuffles the ties. A second application of the same sort toggles its direction. A sort applied while a manual order exists asks before replacing it ([E34](#12-error--state-copy)). (Inherited obligation for the Data Foundation PRD: comparing codes this way wherever they are ordered.) | ⌛️ Ready for Alignment |  |
...
| R7.9 | v1 | P0 | If the store file holding the user's collections has moved or is unavailable, the collection-unavailable state names it and the user finds the file ([E26](#12-error--state-copy)); nothing about the session is kept anywhere else. | ⌛️ Ready for Alignment |  |
...
| ID | Release | Pri | Requirement | Status | Commit PR |
| :--- | :--- | :--- | :--- | :--- | :--- |
| R8.1 | v1 | P0 | The set-aside list is entered four ways: the queue is exhausted with rows set aside, the operator chooses "Review the set-aside swatches" at any point, a session starts on a collection with nothing pending and rows set aside, or a resume lands on a remembered row that was selected there ([R3.8](#3-the-capture-session)). Entering it discards any partial set on the current row, with the discard cue and the count ([R7.1](#7-pause-end-interruption-and-resume), fences F19, F26). Whether the list is the same surface as the recents strip, adjacent to it, or separate is settled with the seam and is part of OQ 8 (fence F20, [§10](#10-the-seam-capture-to-collection)). | ⌛️ Ready for Alignment |  |
...
| R8.5 | v1 | P0 | "Leave it set aside" takes an optional note — the swatch is missing, damaged, or not worth the time today — and the row stays set aside with the decision and its note kept ([E27](#12-error--state-copy)). (Inherited obligation for the Data Foundation PRD.) | ⌛️ Ready for Alignment |  |
...
| R8.15 | v1 | P0 | The review also offers one action that settles everything still outstanding: "Leave all set aside" marks every remaining row as deliberately left, taking one optional note that applies to all of them, and satisfies [R8.6](#8-deferred-row-review-and-corrections)'s completion condition in a single keypress ([E39](#12-error--state-copy), fence F31). Per-row "Leave it set aside" with its own note remains, so a hundred rows flagged missing never need a hundred decisions while a single interesting one can still carry its own reason. Each row keeps its own record of the decision. (Inherited obligation for the Data Foundation PRD.) | ⌛️ Ready for Alignment |  |
...
| R8.10 | v1 | P1 | A re-scan whose samples disagree offers re-take, "Accept the average" — the new set becomes canonical with its spread recorded and the prior reading kept as history (fence F10) — or abandon, which leaves the existing canonical value untouched and keeps the attempt in the row's history. | ⌛️ Ready for Alignment |  |
...
| R9.4 | v1 | P1 | With no session open on the collection, the scan runs as a one-row session: the pre-flight gate runs as it does for any capture ([device PRD UJ2](prd-device-management.md#uj-2-start-acquisition)), the connected instrument is bound, and the session ends itself when the row is captured or deferred, when the attempt is abandoned, or when the operator ends it from a halt or otherwise — so every device halt has a session to belong to and no one-row session is left open to block the next bulk session (fences F12, F17). It never moves the collection's remembered row ([R3.11](#3-the-capture-session)). A capture made while this collection's bulk session is paused is not this case ([R9.6](#9-ad-hoc-capture-and-one-row-sessions)). | ⌛️ Ready for Alignment |  |
...
| R10.2 | v1 | P0 | An always-visible recents strip of the last RECENTS_STRIP_LENGTH captured rows (candidate 5 — OQ 23) on the capture surface makes the just-captured row findable during the session ([R4.15](#4-the-scan-loop)). True under either reading. | ⌛️ Ready for Alignment |  |
...
| R11.2 | v1 | P0 | The capture surface carries an always-visible simulated indicator for the whole session, and every reading taken from the Demo Device is permanently marked simulated ([E35](#12-error--state-copy); [device PRD §6](prd-device-management.md#6-mock-device-layer)). (Handoff to the device PRD.) | ⌛️ Ready for Alignment |  |
| R11.3 | v1 | P0 | The Demo Device's trigger is on-screen or keyboard, since there is no physical button; that it has no physical trigger is one of the device PRD's closed list of simulated-versus-live exceptions ([device PRD §6](prd-device-management.md#6-mock-device-layer)), and whether a real instrument's button reaches the app at all is OQ 2. It paces at DEMO_SCAN_CYCLE — a named provisional constant standing for the real instrument's measured cycle, closed by the device PRD's OQ 10 (this document's OQ 22) — and never instantly, because an instant simulation hides every race the capture surface has. This PRD depends on the device PRD's configurable-latency row, which is P1 there, and pulls it into the first build phase, because the timing promises in [R4.8](#4-the-scan-loop) and [R4.13](#4-the-scan-loop) cannot be measured without it. | ⌛️ Ready for Alignment |  |
...
| R11.11 | v1 | P0 | After any run, a test can read back every row's state, its canonical value, the samples kept on it, every attempt with its cause and its outcome, its version-history entries, the review decisions and notes on it, the queue order, the remembered row with its review-selected mark, and the session chain with each session's status, tallies, instrument, and elapsed capture time. Everything this document promises is kept is therefore checked against what is actually there rather than against what the screen said at the time. (Inherited obligation for the Data Foundation PRD.) | ⌛️ Ready for Alignment |  |
| R11.12 | v1 | P0 | A test can observe which named [§12](#12-error--state-copy) state the app is presenting, and, where a state is written in variants — [E2](#12-error--state-copy)'s empty and finished, [E33](#12-error--state-copy)'s scanned and set aside — which variant is up. A variant is not a state of its own: each state keeps one id however many variants it is written in. A test can also list what the capture surface offers and shows — its actions, the current row, the position counter, the sample counter, the tallies, the recents strip, and the mode indicators. Presence, absence, which state is up, and which variant of it are therefore all assertable without matching any wording, so copy can change without breaking a behavioural test. | ⌛️ Ready for Alignment |  |
| R11.13 | v1 | P0 | A test can put focus on any surface in the app, deliver any capture shortcut, and observe whether it was accepted, rejected, or ignored — which is what makes "capture shortcuts are inert off the capture surface" ([R4.4](#4-the-scan-loop), [R10.4](#10-the-seam-capture-to-collection)) and the mode-slip count of [R10.7](#10-the-seam-capture-to-collection) measurable rather than argued. | ⌛️ Ready for Alignment |  |
| R11.14 | v1 | P0 | A test can see every announcement the app made for assistive technology — its text, when it was made, whether it needed focus to be heard, and whether it cut off one already being read — so the announcement policy of [R4.17](#4-the-scan-loop) can be judged against a real three-second loop. OQ 14 closes against this record. | ⌛️ Ready for Alignment |  |
...
**Labels.** This table is the one place a user-facing label is written. A requirement row that names an action quotes the wording used here, and every action a row names appears in that state's copy below; where the two ever differ, this table is right.

**Placeholders.** ⟨…⟩ stands for something filled in when the state is shown. A sentence whose count would be zero is left out rather than shown as "0". A sentence carrying a count agrees with it — "1 swatch", "⟨n⟩ swatches". A named provisional constant renders as the number it currently holds, never as its name. ⟨code⟩ always names the swatch the operator will actually land on, so where the remembered row is no longer pending it is the row the fallback rule picks ([R3.7](#3-the-capture-session)). ⟨dropped⟩ is the number of samples let go by the action that led here ([R7.1](#7-pause-end-interruption-and-resume), fence F26).
...
| E2 | Nothing to capture | Empty: "There's nothing to scan in this collection yet" · Finished: "Everything in ⟨collection⟩ is scanned" | Empty: "Import your swatch list from a spreadsheet, or add one swatch at a time." · Finished: "Every swatch here has a reading. Add more swatches, or scan one again if something needs correcting." | Empty: Import a file; Add a swatch · Finished: Add a swatch; Re-scan a swatch | ⌛️ Ready for Alignment |
| E3 | Instrument held by another collection | ⟨collection⟩ is using the instrument | One session at a time. Finish or end the session in ⟨collection⟩, then come back here. | Go to ⟨collection⟩ | ⌛️ Ready for Alignment |
...
| E5 | Can't read the file | Can't read this file | The text in this file isn't in a form the app recognizes. Try saving it again from your spreadsheet as CSV, or choose the text encoding yourself. Nothing has been imported. | Choose an encoding; Pick a different file | ⌛️ Ready for Alignment |
...
| E11 | Ambiguous code | ⟨code⟩ matches more than one swatch in this collection | This swatch is left out rather than guessed at. Nothing in your collection changes. | Continue without it | ⌛️ Ready for Alignment |
...
| E18 | Samples disagree | These readings don't agree | The samples of this swatch came out further apart than expected. Take it again, accept the average of what you have, or set it aside for the end. Every reading is kept either way — nothing is averaged away without you. | Take it again; Accept the average; Set it aside | ⌛️ Ready for Alignment |
| E19 | Guard paused | Several swatches in a row haven't worked | ⟨n⟩ in a row have gone wrong — either ⟨n⟩ scans the instrument refused, or ⟨n⟩ swatches it set aside. It might be how the instrument is sitting, the light in the room, its temperature, or the swatches themselves if they're textured or metallic. Your swatch and its samples are still here. You can run a calibration without leaving this pause, and you come back and resume when you're ready. | Check placement and resume; Recalibrate | ⌛️ Ready for Alignment |
| E20 | Flag has nothing to demote | ⟨code⟩ is already set aside | The swatch you just left is waiting for you at the end. There's nothing here to flag. | None | ⌛️ Ready for Alignment |
...
| E22 | Paused | Paused | ⟨captured⟩ scanned, ⟨deferred⟩ set aside, ⟨pending⟩ to go. The trigger is off while you're paused. Resuming starts ⟨code⟩ again from its first sample; the ⟨dropped⟩ samples you'd taken on it were let go. | Resume | ⌛️ Ready for Alignment |
| E23 | End early | Stop here for now? | What you've scanned stays scanned. Anything set aside stays set aside, and anything you haven't reached stays waiting. A part-finished swatch starts over next time. ⟨deferred⟩ swatches are set aside — you can deal with them now or another day. Your next session picks up at ⟨code⟩. | End session; Review them now — only while any are set aside; Keep scanning | ⌛️ Ready for Alignment |
| E24 | Session complete | Done — ⟨captured⟩ swatches in ⟨elapsed⟩ | That's ⟨rate⟩ an hour. ⟨deferred⟩ swatches are set aside, waiting for you. Everything else is in your collection. *(Not copy: where this state lives follows the seam — fence F20, OQ 8.)* | Open the collection; Review the set-aside swatches — the second only while any are set aside | ⌛️ Ready for Alignment |
...
| E26 | Collection unavailable | Can't find the file holding your collections | It's been moved, renamed, or it's on a drive that isn't connected. Find it and the app will pick up exactly where you were. | Find the file | ⌛️ Ready for Alignment |
| E27 | Leave one swatch set aside | Leave ⟨code⟩ set aside? | Add a note if you like — missing, damaged, or just not today. It stays in your collection, marked, with everything the instrument did read. | Leave it set aside; Try again | ⌛️ Ready for Alignment |
| E28 | Re-scan offered | ⟨code⟩ already has a reading | Scanning it again makes the new reading its value and keeps the old one in its history. You won't see the old reading while you scan. The ⟨dropped⟩ samples you'd taken on the swatch you were on were let go to come here. | Re-scan; Cancel | ⌛️ Ready for Alignment |
...
| E32 | Add item cancelled | Nothing was added | You're back on ⟨code⟩. It starts again from its first sample; the ⟨dropped⟩ samples you'd taken on it were let go. | None | ⌛️ Ready for Alignment |
| E33 | Reorder refused | ⟨code⟩ isn't waiting to be scanned | Only swatches still waiting in the queue can be moved. This one is ⟨scanned / set aside⟩. | Scanned: Re-scan it; Cancel · Set aside: Review the set-aside swatches; Cancel | ⌛️ Ready for Alignment |
...
| E35 | Simulated readings | Demo Device — readings are generated, not measured | Everything works as it does with a real instrument, apart from a short list of hardware-only differences, and every reading is permanently marked as simulated. | None (always visible) | ⌛️ Ready for Alignment |
...
| E37 | No matching row | Nothing here matches "⟨text⟩" | The app looks at the swatch code and name and at the alternate code and name. Check what you typed, or add this swatch to the collection now. | Add a swatch; Keep looking | ⌛️ Ready for Alignment |
| E38 | File gone before the import | That file isn't where it was | It's been moved, renamed, or deleted since you picked it. Nothing has been imported. Pick it again and the app will read it fresh. | Pick the file again; Cancel | ⌛️ Ready for Alignment |
| E39 | Leave every remaining swatch set aside | Leave the last ⟨n⟩ swatches set aside? | They stay in your collection, marked, with everything the instrument did read. Add one note for all of them if you like. This finishes the session. | Leave them all set aside; Go back | ⌛️ Ready for Alignment |
| E40 | Import blocked by a session | Finish your session in ⟨collection⟩ first | A session is open here, so new swatches can't join its queue mid-run. End or finish the session, then import — nothing about the file has changed. | Go to the session; End that session; Cancel | ⌛️ Ready for Alignment |
| E41 | Matched on an alternate | Found ⟨name⟩ by its ⟨alternate code / alternate name⟩ | "⟨text⟩" isn't this swatch's own code — it's the alternate you gave it. Its code is ⟨code⟩. | Scan it; Keep looking | ⌛️ Ready for Alignment |


Inherited obligations this PRD hands forward: the Data Foundation PRD owns what the app keeps, how nothing is ever destroyed, and how a test reads it all back ([R1.9](#1-collections), [R2.7](#2-inventory-import), [R2.10](#2-inventory-import), [R4.6](#4-the-scan-loop), [R4.11](#4-the-scan-loop), [R4.12](#4-the-scan-loop), [R4.13](#4-the-scan-loop), [R3.1](#3-the-capture-session), [R3.6](#3-the-capture-session), [R3.8](#3-the-capture-session), [R6.7](#6-queue-navigation-and-reordering), [R6.9](#6-queue-navigation-and-reordering), [R7.12](#7-pause-end-interruption-and-resume), [R8.5](#8-deferred-row-review-and-corrections), [R8.9](#8-deferred-row-review-and-corrections), [R8.13](#8-deferred-row-review-and-corrections), [R8.15](#8-deferred-row-review-and-corrections), [R11.6](#11-demo-device-and-verifiability), [R11.10](#11-demo-device-and-verifiability), [R11.11](#11-demo-device-and-verifiability)); the Collection Mode PRD owns rename, delete, renaming an imported column, the simulated-readings banner, and showing a recorded spread ([R1.8](#1-collections), [R2.6](#2-inventory-import), [R4.11](#4-the-scan-loop), [R8.14](#8-deferred-row-review-and-corrections)), and it also owns the sort and drag controls on the collection view that the operator uses to put the queue in order before a session, while this PRD keeps what a reorder does to the queue ([R6.8](#6-queue-navigation-and-reordering), fence F22); the QC & Comparison PRD owns the check-it path ([R8.11](#8-deferred-row-review-and-corrections)); the device PRD's simulated layer gains four obligations ([R11.5](#11-demo-device-and-verifiability), the device half of [R11.6](#11-demo-device-and-verifiability), [R11.7](#11-demo-device-and-verifiability), [R11.8](#11-demo-device-and-verifiability)) and its §5 gains one note ([R7.14](#7-pause-end-interruption-and-resume)). These are requirements on those documents, not suggestions. Device-path PRs still need a human with real hardware to verify before merge ([R11.10](#11-demo-device-and-verifiability)); these rows describe product behaviour only, and module layout is an open ADR ([AGENTS.md §3](../../AGENTS.md#3-decided--recommended--open)) that is not decided here.
...
| 8 | The seam | Reading A, capture as a modal takeover that hands off at session end, or Reading B, capture as a state of the live collection. The data behaviour is common to both ([R10.1](#10-the-seam-capture-to-collection)), so this is a navigation question only. The research default is Reading B. Two surface questions are folded in and settled with it (fence F20): where the deferred-row review lives relative to the recents strip — same surface, adjacent, or separate (was OQ 10) — and whether the session summary is a state or a dialog (was OQ 17), specified meanwhile as a state on the collection surface so the counts and the items-per-hour stay readable. | The owner's product call on a Demo Device prototype of each reading, measured against [R10.7](#10-the-seam-capture-to-collection). The prototype is first-build work rather than a later exercise (fence F25), and the same call settles the review's surface and the summary's form (fence F20). PROTOTYPE_PARTICIPANTS = 1: the owner walks the script on both readings (fence F35); the tie-break is pre-committed in R10.7. | R10.1–R10.7, R6.12, R8.1, R4.15, R7.5, R7.11, R7.15, M2 | owner | open | owner | — |
...
| 21 | Which illuminant and observer pairs may a collection choose? | The reference set is the instrument toolkit's — the full eighteen reference whites on Spectro-class hardware ([SDK audit](../briefs/nix-universal-sdk-audit-findings.md), per SDK docs), and D50/2° alone without the spectral entitlement. What is open is the exact list and how the entitlement narrows it. The agreement check is unaffected either way: fence F28 fixes it at D50/2°. | Read the toolkit's reference set on hardware, with and without the spectral entitlement | R1.5 | SDK docs | open | hardware | — |
