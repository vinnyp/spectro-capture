...
2. If no collection is selected, select a collection. The queue is the collection's pending rows in queue order (import order until the operator reorders, [UJ3.10](#uj-310-reorder-the-queue)); the session opens at the collection's remembered row, or at the next pending row in queue order if that row is no longer pending (fence F15; a row that was selected in the review opens the review at that row instead, for as long as that row is still deferred — vocabulary above) — a collection that has never had a session opens at its first pending row
...
- The remembered row also carries whether the operator chose it in the review, and that mark clears as soon as the row is no longer deferred ([vocabulary](#user-journeys); [UJ3.3](#uj-33-resolve-the-deferred-error-queue-at-session-end) step 2; fence F15)
