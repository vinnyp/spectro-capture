...
1. The queue is exhausted with rows set aside, or I choose "Review the set-aside swatches", or I started a session with only deferred rows waiting. I see every one with its code, name, why it was set aside, how many attempts were made, and how many samples it kept.
...
The Commit PR column is where a row maps onto the work that lands it. Owner decisions F1–F38 are recorded in the [fence file](prd-capture-mode-fences.md); a row that implements a fence cites it, and no row re-argues one.
...
| Capture surface | The current row, the position in the queue, the sample counter, the session tallies, the recents strip, and the always-visible simulated indicator | Light leaked in; out of temperature range; moved on; samples disagree; guard paused; flag has nothing to demote; re-take unavailable; samples let go; paused; wrap exhausted; end early; every halt state (device PRD §5); simulated readings |
...
| Deferred-row review (the set-aside list) | Every set-aside row with its cause, how many attempts were made, and how many samples it kept | Leave one swatch set aside; leave every remaining swatch set aside; samples disagree; guard paused; samples let go |
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| R1.1 | v1 | P0 | Creating a collection is an explicit user action and ends with the collection ready and its queue empty. Collections are flat: there is no library above them (fence F1). From there the user imports an inventory ([§2](#2-inventory-import)) or adds items one at a time ([§9](#9-ad-hoc-capture-and-one-row-sessions)). "Start capture session" is offered once the collection has at least one pending or deferred row. | 🤝 Aligned |  |
| R1.2 | v1 | P0 | A collection's name is required and must be unique among the user's collections, compared by the one matching rule ([R2.7](#2-inventory-import), fence F11); a duplicate shows [E1](#12-error--state-copy) and the collection is not created. A test can create two collections whose names differ only in letter case or in spacing, observe the second refused, and read back that only one collection exists ([R11.11](#11-demo-device-and-verifiability)). | 🤝 Aligned |  |
| R1.3 | v1 | P0 | Samples-per-row is required at creation, arrives pre-filled at 3, and accepts 1 to 5 (vision [U1](vision.md#use-cases), fence F2). It is the collection's default for every session and is changeable between sessions; whether it can change mid-session is OQ 9. | 🤝 Aligned |  |
| R1.4 | v1 | P0 | With samples-per-row set to 1 there is nothing to compare, so the agreement check ([R4.9](#4-the-scan-loop)) does not run (fence F10). A test can set a collection to one sample per row and observe no agreement check and no disagreement state. | 🤝 Aligned |  |
| R1.5 | v1 | P0 | Illuminant and observer are optional collection-level display defaults that arrive pre-filled at D50/2°, are never required at creation, and are editable at any time (fences F2, F28). Changing them never requires a re-scan, never alters a stored reading, and never changes an agreement verdict ([R4.9](#4-the-scan-loop), fence F28), because the raw measurement is what the app keeps and colour values are worked out from it ([AGENTS.md §8](../../AGENTS.md#8-vocabulary)). The pairs the user may choose from are the reference set the instrument's own toolkit offers — the full eighteen reference whites on Spectro-class hardware ([SDK audit](../briefs/nix-universal-sdk-audit-findings.md), per SDK docs; confirm on hardware — OQ 21); without the spectral entitlement only D50/2° is available, the average is taken across the instrument's colour values instead of its curves and the reading is marked non-spectral ([R4.12](#4-the-scan-loop), fence F36), and the agreement check runs under D50/2° in either case (fence F28). | ⌛️ Ready for Alignment — edited in round 6, re-review |  |
| R1.6 | v1 | P0 | The ISO 13655 measurement condition (M0/M1/M2) is a scan mode of the instrument, not an observer, and which scan modes a capture records is OQ 1 (Spectro 2 supports M0/M1 plus M2 on F2.x firmware — [SDK audit](../briefs/nix-universal-sdk-audit-findings.md), per SDK docs; confirm on hardware). Until OQ 1 closes the app keeps every mode a reading arrives with and uses M1 as the collection's scan mode ([R1.10](#1-collections)), so nothing is discarded while the question is open. | 🤝 Aligned |  |
| R1.7 | v1 | P0 | A collection with nothing pending, and no set-aside row still unsettled, shows the nothing-to-capture state ([E2](#12-error--state-copy)) in one of two variants: truly empty, offering import and add-a-swatch; or finished — every row either captured or deliberately left set aside — saying so and additionally offering a re-scan ([§8](#8-deferred-row-review-and-corrections)). A row the operator deliberately left set aside is settled and counts as dealt with, so a collection can be finished with rows in it that were never captured (fence F31 as clarified, [R8.5](#8-deferred-row-review-and-corrections), [R8.15](#8-deferred-row-review-and-corrections)). A collection with nothing pending but an unsettled set-aside row waiting is not this state at all: starting capture there opens straight into the set-aside list ([R3.9](#3-the-capture-session)). Nothing else happens. | ⌛️ Ready for Alignment |  |
| R1.8 | v1 | P0 | Renaming and deleting a collection belong to Collection Mode (fence F3), carrying the name-uniqueness rule of R1.2, a delete guard that requires an active or interrupted session to end first, and a delete warning that names the count of captured rows and their history and offers an export before deleting — permitted, because "corrections never destroy data" is about measurements rather than containers ([AGENTS.md §4](../../AGENTS.md#4-non-negotiables)), but never the default action. (Inherited obligation for the Collection Mode PRD.) | 🤝 Aligned |  |
| R1.9 | v1 | P0 | A collection's rows, their queue order, its remembered row, and its sessions all live with the collection and nowhere else — no part of the work is kept somewhere the collection does not reach. A test can start the app from a clean state with only the store file present and read back the rows, the queue order, the remembered row, and the session chain unchanged ([R11.11](#11-demo-device-and-verifiability)). How many store files a user's collections are spread across is the Data Foundation PRD's to settle (OQ 19). (Inherited obligation for the Data Foundation PRD.) | 🤝 Aligned |  |
| R1.10 | v1 | P0 | A collection carries one chosen scan mode — M0, M1, or M2, whichever the instrument's firmware offers ([SDK audit](../briefs/nix-universal-sdk-audit-findings.md), per SDK docs; confirm on hardware — OQ 1) — set at creation beside samples-per-row, arriving pre-filled at M1 on the Spectro 2, and changeable between sessions exactly as samples-per-row is (fence F34). It is the mode the agreement check compares ([R4.9](#4-the-scan-loop)) and the mode a colour value is worked out from unless the user asks for another. Every mode a reading arrives with is kept whatever the collection has chosen ([R4.5](#4-the-scan-loop)), so changing the choice never asks for a re-scan and never alters a reading already taken; it does change what the agreement check compares from then on, which is why it is a between-sessions setting and not a mid-session one. A test can create a collection, read back M1, change the mode between two sessions, and read back both readings still carrying every mode they arrived with ([R11.11](#11-demo-device-and-verifiability)). | 🤝 Aligned |  |
...
| ID | Release | Pri | Requirement | Status | Commit PR |
| :--- | :--- | :--- | :--- | :--- | :--- |
| R2.1 | v1 | P0 | Import starts from an explicit user action and accepts a file by picker or by drag and drop. | 🤝 Aligned |  |
| R2.2 | v1 | P0 | The app shows what it detected — the header row and the row count. The encoding and the delimiter are shown only when detection failed or when the user asks, because a cataloger should not have to know those words. The detection rules themselves, and whether a remembered mapping becomes a named template the user manages, are OQ 12; the research documents only that file reading can be configured for encoding, delimiter, and malformed rows ([acq v1 §7](../briefs/acquisition-experience-research-results.md)). | 🤝 Aligned |  |
| R2.3 | v1 | P0 | Four read failures are each a distinct named state that ends the import with nothing imported: no header row detected ([E4](#12-error--state-copy)), where the user names the columns or picks the header row manually; text that cannot be read ([E5](#12-error--state-copy)), where the user chooses an encoding or re-saves the file; a file with zero data rows ([E6](#12-error--state-copy)); and a file that has moved, been renamed, or gone away between being picked and being committed ([E38](#12-error--state-copy)), which is its own state and offers the file picker again rather than an encoding the user never got wrong. | 🤝 Aligned |  |
| R2.4 | v1 | P0 | Rows with the wrong number of columns are listed by row number and excluded, never silently imported ([E7](#12-error--state-copy)); the user proceeds without them or fixes the file and re-picks it. | 🤝 Aligned |  |
| R2.5 | v1 | P0 | The target collection — new or existing — is chosen in the app before columns are mapped, and the collection name is never a mapped column in v1 (fence F4). A new collection collects the same name, samples-per-row, scan mode, and optional display defaults as [§1](#1-collections). | 🤝 Aligned |  |
| R2.6 | v1 | P0 | Swatch Code must be mapped; without it the mapping cannot be saved ([E8](#12-error--state-copy)). Swatch Name, Swatch Alternate Code, and Swatch Alternate Name are optional; every other column is imported as extra metadata the collection can surface, and a column with a blank header arrives under a generated name listed in the preview ([E12](#12-error--state-copy)) for the user to rename later. (Inherited obligation for the Collection Mode PRD: renaming an imported column.) | 🤝 Aligned |  |
| R2.7 | v1 | P0 | The one matching rule (fence F11): two Swatch Codes — or two collection names — count as the same when they match after the text is brought to one standard form, spaces at either end are trimmed, any run of space inside is collapsed to one, and letter case is ignored the same way in every language the app runs in. Any character the user's keyboard produces as a space — an ordinary space, a tab, a non-breaking space — counts as a space. The value is always shown exactly as it was entered. The rule governs uniqueness, re-import matching, find, the ad-hoc duplicate check, and collection names, so a re-export from Numbers or an Excel autocorrect never doubles the queue. Two codes that are distinct in the file but merge under this rule are both excluded and listed, deliberately ([R2.8](#2-inventory-import)) — the app never picks a winner. A test can import "CG 3" and "cg  3" — the same letters with an extra space — and read back one row; the same recipe with an accented code entered two ways, with a tab, and with a non-breaking space gives the same answer ([R11.11](#11-demo-device-and-verifiability)). (Inherited obligation for the Data Foundation PRD: comparing stored codes by this rule.) | 🤝 Aligned |  |
...
| R2.9 | v1 | P0 | A preview precedes every commit: the row count — for an existing collection the three-way count new / updated / unchanged plus a line reading "N rows in the collection are not in this file — left untouched" — and the issue list. The count is the reconciliation interface ([acq v2 §7 Q7.5](../briefs/acquisition-experience-research-results-v2.md#7-inventory-import-and-column-mapping)). A file that changed on disk between being read and being committed is re-read and re-previewed ([E13](#12-error--state-copy)). A file that would take the collection past ROWS_CEILING ([R3.12](#3-the-capture-session)) is warned about in the preview, naming the number; the import is still allowed. | 🤝 Aligned |  |
| R2.10 | v1 | P0 | A commit is all-or-none, so a failure partway leaves the collection exactly as it was; a test can make the commit fail and read back a collection identical to the one before it ([R11.11](#11-demo-device-and-verifiability), [device PRD §6](prd-device-management.md#6-mock-device-layer): the store can be made to fail on commit). Every imported row lands pending, in file order, appended after any existing rows. Importing into a collection whose session is active, paused, or interrupted is refused, naming that session and offering two ways past it — going to the session, and ending it from here without going anywhere — so a blocked import is never a dead end ([E40](#12-error--state-copy)) — the simpler rule, chosen over letting new rows appear behind the operator mid-run. The mapping is remembered by the file's header signature — the set of column names, in any order, each compared by the one matching rule ([R2.7](#2-inventory-import)); a generated name for a blank header carries its column's position, so the same blank header in a different column is a different signature — and pre-filled next time, so the user confirms rather than re-maps. Import ends at ready-to-capture and never starts a session. (Inherited obligation for the Data Foundation PRD: an import that either lands whole or not at all.) | 🤝 Aligned |  |
...
| ID | Release | Pri | Requirement | Status | Commit PR |
| :--- | :--- | :--- | :--- | :--- | :--- |
| R2.11 | v1 | P0 | Swatch Code is how a file row is matched to a collection row; the code is the way in, not the row itself, so a matched row keeps its place, its state, and its measurements whatever the file says ([acq v2 §7 Q7.3](../briefs/acquisition-experience-research-results-v2.md#7-inventory-import-and-column-mapping)). Unchanged rows are untouched, so importing the same file twice changes nothing, a re-import never deletes a row, and rows the file does not mention are left exactly as they are. | 🤝 Aligned |  |
| R2.12 | v1 | P0 | A code in the file that matches more than one row in the collection is a hard failure listed in the preview ([E11](#12-error--state-copy)), neither created nor updated — an ambiguous key is never a guess. Codes unique within a collection make this unreachable; the row exists so the guarantee is stated. | 🤝 Aligned |  |
| R2.13 | v1 | P0 | Where an updated row is already captured, the preview offers a keep-or-overwrite toggle on each such row with one default applied to all of them, never a series of dialogs ([E14](#12-error--state-copy)). The default is to take the new details, because the ordinary reason to re-import is that the user corrected the spreadsheet; a mapped column that is blank in the file participates in that default and clears the field, and the preview says so in as many words ([R2.14](#2-inventory-import)). The state also states that measurements are never touched by a metadata change — the canonical value and its version history are unaffected either way ([acq v2 §7 Q7.5](../briefs/acquisition-experience-research-results-v2.md#7-inventory-import-and-column-mapping)). | 🤝 Aligned |  |
| R2.14 | v1 | P0 | A mapped column present in the file but blank for a row is treated as a change, because it would clear the field, and appears under "updated" so the user sees it; a column omitted from the file entirely leaves existing values untouched. | 🤝 Aligned |  |
...
| ID | Release | Pri | Requirement | Status | Commit PR |
| :--- | :--- | :--- | :--- | :--- | :--- |
| R3.1 | v1 | P0 | A capture session belongs to exactly one collection and records the instrument it is using, when it started and ended, its status, and how much capture time has elapsed (fence F12 as amended by fence F15). Elapsed capture time is the sum of the stretches in which the session was actually capturing; the operator's pause, the guard's pause, every device halt, time spent in the set-aside list, and the gap between an interruption and the resume that follows it are all outside it, so the number stands for work done rather than time passed. A test can drive a session through each of those and read back an elapsed capture time that counts only the capturing stretches ([R11.11](#11-demo-device-and-verifiability)). (Inherited obligation for the Data Foundation PRD.) | 🤝 Aligned |  |
| R3.2 | v1 | P0 | A session's status is exactly one of active, interrupted, ended, or complete (fence F12). At most one session is active in the app at a time; a collection holds at most one unresumed interrupted bulk session; two collections may each hold one (fences F12, F14). A capture made on a paused bulk session brings no second session record into being ([R3.11](#3-the-capture-session), [R9.6](#9-ad-hoc-capture-and-one-row-sessions)). A test can construct each status, read it back, and assert the two limits ([R11.6](#11-demo-device-and-verifiability), [R11.11](#11-demo-device-and-verifiability)). | 🤝 Aligned |  |
| R3.3 | v1 | P0 | The instrument binding is released when the app quits or crashes, so no session outlives the app holding an instrument against another collection (fence F12; [device PRD §1](prd-device-management.md#1-device-pairing)). | 🤝 Aligned |  |
| R3.4 | v1 | P0 | A session becomes interrupted only by a quit while capturing or while paused, a crash, a force-quit, or power loss. System sleep is a device halt and the session stays active through it; quitting from a device halt is the device PRD's End session and ends the session instead, while a crash, force-quit, or power loss during a halt does interrupt it (fence F14; [device PRD §5](prd-device-management.md#5-mid-session-device-failure)). | 🤝 Aligned |  |
| R3.5 | v1 | P0 | Starting capture while another collection's session is active — including while that session is paused or halted — is refused, naming the collection whose session holds the instrument ([E3](#12-error--state-copy)); an interrupted session on another collection holds no instrument and never blocks. Starting capture on a collection whose session is already in flight returns to that session rather than starting a second — one in-flight session per collection, always ([acq v2 §8 Q8.4](../briefs/acquisition-experience-research-results-v2.md#8-progress-orientation-and-session-completion)). | 🤝 Aligned |  |
| R3.6 | v1 | P0 | The remembered row belongs to the collection rather than to a session, and updates every time the queue advances, the operator jumps, an item is inserted, or a deferred row is selected in the review (fence F15). (Inherited obligation for the Data Foundation PRD.) | 🤝 Aligned |  |
| R3.7 | v1 | P0 | Every session on a collection opens at the remembered row — whether the last session ended early, was interrupted, or completed — falling back to the next pending row in queue order if that row is no longer pending, and, where there is no pending row at all, into the set-aside list ([R3.9](#3-the-capture-session)) while any set-aside row is still unsettled, or the nothing-to-capture state ([R1.7](#1-collections)) when every one of them has been deliberately left (fence F15, fence F31 as clarified). A jump or a reorder made yesterday is honoured today and nobody is ever sent back to the top of the queue. A collection that has never had a session opens at its first pending row. | ⌛️ Ready for Alignment |  |
...
| R3.9 | v1 | P0 | A session started on a collection with nothing pending and one or more unsettled set-aside rows runs the pre-flight gate and opens straight into the review, so a second day with only set-aside rows is never stranded ([§8](#8-deferred-row-review-and-corrections)). Rows the operator deliberately left set aside are settled and never reopen the review: where every set-aside row is settled the collection is finished and shows [E2](#12-error--state-copy)'s finished variant instead ([R1.7](#1-collections), fence F31 as clarified). Either way a settled row can still be re-scanned by choice, from the collection or from the set-aside list ([§8](#8-deferred-row-review-and-corrections)). | ⌛️ Ready for Alignment |  |
| R3.10 | v1 | P0 | The first capture action against the queue runs the device PRD's pre-flight gate — battery, calibration currency, authorization window, storage headroom, and the muted-audio advisory ([device PRD §4](prd-device-management.md#4-pre-flight-device-health)); a blocking check leaves the session unstarted and the queue untouched. Every check runs again every time; what a repeat suppresses is only the presentation of a gate whose checks all pass, so a check that has begun to block or advise always surfaces and adding several ad-hoc items in a row does not re-show a passing gate. | 🤝 Aligned |  |
| R3.11 | v1 | P0 | A session binds whichever instrument is connected when it starts ([device PRD §1](prd-device-management.md#1-device-pairing)). "One-row session" names exactly two cases: a single capture on a collection with no session open, and one beside an unresumed interrupted bulk session ([R9.7](#9-ad-hoc-capture-and-one-row-sessions), fence F17) — a quick capture on a paused bulk session is a row inside that session and not a session of its own ([R9.6](#9-ad-hoc-capture-and-one-row-sessions)). A one-row session ends itself when its row is captured or deferred, when the attempt is abandoned, or when the operator ends it, and is never left open to block the next bulk session (fence F12); one that is itself cut short is closed as ended on relaunch and never counts against the one-unresumed-bulk-session rule (fence F17). It never changes the collection's remembered row, so a quick correction never moves where the next bulk session opens ([R3.6](#3-the-capture-session), fence F15). | 🤝 Aligned |  |
| R3.12 | v1 | P0 | The design scale, as named provisional constants: ROWS_TARGET (candidate 200–1,200 rows per collection and per import — the marker set and the swatch book — OQ 13), ROWS_CEILING (candidate 10,000 rows, the bound the ADR-0001 spike tested — OQ 13), and SESSION_LENGTH, the longest single session the design must hold up under (candidate one working day — OQ 13). Each is bound to something a test can check: find and the queue list answer within FIND_BUDGET on a collection at ROWS_CEILING (candidate TBD — OQ 13); an import of ROWS_TARGET rows commits within IMPORT_BUDGET (candidate TBD — OQ 13); and a file that would take a collection past ROWS_CEILING is warned about at preview and imported anyway ([R2.9](#2-inventory-import)). SESSION_LENGTH is bound the same way: a session of that length on a collection of ROWS_TARGET rows, paced at DEMO_SCAN_CYCLE ([R11.3](#11-demo-device-and-verifiability)), ends with its tallies, its elapsed capture time, and its queue order intact, and with TRIGGER_ACK_WINDOW ([R4.8](#4-the-scan-loop)) still met on the last row of the run — so "holds up under a working day" is something a test can sit through rather than a claim. | ⌛️ Ready for Alignment |  |
...
| R4.3 | v1 | P0 | A trigger press while a scan is in flight is rejected — never queued, never silently dropped — with immediate non-visual feedback, a rejection tone or haptic distinct from success and from caution. The lockout lasts until the reading in flight returns or fails, however long that is (fence F32); a fixed window shorter than the instrument's own cycle would accept a second press mid-measurement. LOCKOUT_WINDOW is a named provisional constant for any additional dead time that follows the reading (candidate 0.5 s, from the barcode-scanner precedent, [acq v2 §3 Q3.1](../briefs/acquisition-experience-research-results-v2.md#3-pacing-latency-and-feedback)), and OQ 4 is narrowed to whether there should be any such dead time at all. A test can hold a reading open for as long as it likes, press the trigger repeatedly, and observe every press rejected and no second measurement asked for ([R11.5](#11-demo-device-and-verifiability)). | 🤝 Aligned |  |
| R4.4 | v1 | P0 | No capture shortcut is a bare Space or a bare single letter, and capture shortcuts are live only while the capture surface has focus ([acq v2 §2 Q2.2](../briefs/acquisition-experience-research-results-v2.md#2-heads-down-input-and-interaction-modality)); a test can put focus anywhere in the app, send any capture shortcut, and observe whether it was accepted, rejected, or ignored ([R11.13](#11-demo-device-and-verifiability)). The rest of the keyboard is queue management — "Skip", "Flag", find, "Add a swatch", "Re-take sample", "Restart item", "Pause", "End session" ([§12](#12-error--state-copy) Labels) — each a dedicated action distinct from the trigger ([acq v1 §2](../briefs/acquisition-experience-research-results.md)). | ⌛️ Ready for Alignment — edited in round 6, re-review |  |
| R4.5 | v1 | P0 | A reading arrives as one measurement per scan mode ([SDK audit](../briefs/nix-universal-sdk-audit-findings.md), per SDK docs; confirm on hardware); every mode a reading arrives with is kept, and which modes exist for a given instrument is OQ 1. Which of those modes the agreement check compares, and which one a colour value is worked out from unless the user asks for another, is the collection's chosen scan mode ([R1.10](#1-collections), fence F34). | 🤝 Aligned |  |
...
| R4.9 | v1 | P0 | With two or more samples the app checks that they agree within SAMPLE_TOLERANCE — the largest ΔE2000 distance of any one sample from the set's mean ([R4.12](#4-the-scan-loop), fence F33, or from the colour-value mean where the license carries no spectral entitlement — fence F36), measured on the collection's chosen scan mode ([R1.10](#1-collections), OQ 1) and always computed under a fixed D50/2° reference whatever the collection's display defaults happen to be (fence F28). A distance exactly equal to SAMPLE_TOLERANCE agrees; only a larger one disagrees. The reference used, the statistic, and the spread are all recorded with the reading, so an old verdict can be read back and understood (candidate ΔE2000 2.0, a placeholder with no evidence behind the value; the statistic is a candidate too — OQ 3). With one sample the check is skipped (fence F10). No shipped precedent exists for this check; it is designed here, not inherited. | ⌛️ Ready for Alignment — edited in round 6, re-review |  |
| R4.10 | v1 | P0 | A disagreeing set is never silently averaged. A caution reaches the operator through two senses, the row holds (fence F6), and three choices are offered, none of them silent ([E18](#12-error--state-copy)): re-take the item on the spot, "Accept the average", or Skip, which sets the row aside with all its samples and advances exactly once ([§5](#5-per-scan-failure-and-the-consecutive-failure-guard)). A re-taken set that disagrees again counts as a failed attempt toward K_FAILED_ATTEMPTS. The check has the same explicit setting as the guard ([R5.12](#5-per-scan-failure-and-the-consecutive-failure-guard), fence F27): record-only, in which every set is accepted and its spread recorded with no prompt at all, through the dogfood phase; and enabled, offering these three choices, at v1 once OQ 3's data has tuned SAMPLE_TOLERANCE. Which setting a build starts in is an input a test can set, so both paths are walked without hardware. OQ 6 covers this setting and the guard's together. | 🤝 Aligned |  |
| R4.11 | v1 | P0 | "Accept the average" keeps the set exactly as measured and records the spread between its samples on the reading, so a textured, fabric, or metallic swatch can still reach captured (fence F10). (Inherited obligation for the Data Foundation PRD; showing the spread is an inherited obligation for the Collection Mode PRD.) | 🤝 Aligned |  |
| R4.12 | v1 | P0 | A row's measurement is the full set of its N readings, each exactly as the instrument produced it, and that set is saved before anything else happens ([device PRD §5](prd-device-management.md#5-mid-session-device-failure): the save completes before the queue advances). The average is the mean of the samples' reflectance curves, and every colour value the operator sees — including the ones the agreement check compares — is worked out from that mean under the fixed reference of [R4.9](#4-the-scan-loop) (fence F33). Where the license carries no spectral entitlement there are no curves to average and capture still runs: the mean is taken across the colour values the instrument returns, under the same fixed reference, and the reading is marked non-spectral so a collection that holds both kinds says which is which (fence F36; [device PRD §2](prd-device-management.md#2-licensing--pre-authorization) lets a valid license without spectral data reach capture). The basis the average was taken on — curves or colour values — and the version of the working-out are recorded with the reading so the same average can be arrived at again years later, and no individual reading is ever discarded in favour of the average ([AGENTS.md §8](../../AGENTS.md#8-vocabulary)). (Inherited obligation for the Data Foundation PRD, including the non-spectral mark.) | ⌛️ Ready for Alignment |  |
| R4.13 | v1 | P0 | The row-success confirmation, distinct from the per-sample confirm, reaches the operator through two senses only once the set is safely saved, within ROW_CONFIRM_BUDGET of the last sample's arrival (candidate TBD — OQ 5). The promise behind it: a row the operator was told landed survives a power cut or a drive pulled a moment later, because the confirmation waits for the save to be truly complete rather than merely under way. ROW_CONFIRM_BUDGET can therefore never be smaller than what a safe save costs on the volume the user's collection is actually on, so it is measured on each class of volume a cataloger uses — an internal drive, a USB external drive, and a network volume — and not on the Demo Device alone (fence F29). A test can take the drive away, or lose everything not yet safely written, at any chosen moment and read back every row the operator was told landed ([R11.10](#11-demo-device-and-verifiability)). (Inherited obligation for the Data Foundation PRD, ADR-0003.) | 🤝 Aligned |  |
| R4.14 | v1 | P0 | On that confirmation the queue advances on its own to the next pending row and the row appears on the recents strip. A caution tone always pre-empts a success tone in flight ([acq v2 §4 Q4.1](../briefs/acquisition-experience-research-results-v2.md#4-mid-run-error-handling)). | ⌛️ Ready for Alignment — edited in round 6, re-review |  |
| R4.15 | v1 | P0 | The capture surface shows the current row's Swatch Code and Swatch Name; the position "row r of R", where r is the row's place in the full queue order and R is the number of rows in the queue, so the pair reads the same as the queue list does; the sample counter "sample n of N"; the tallies captured / deferred / pending, which are where the remaining work is read; and the recents strip of the last RECENTS_STRIP_LENGTH captured rows (a named provisional constant, candidate 5 — OQ 23) ([acq v2 §11 Q11.2](../briefs/acquisition-experience-research-results-v2.md#11-the-seam--capture-to-collection)). An insert grows R and moves r; a reorder moves both. Where the recents strip sits relative to the set-aside list is settled with the seam and is part of OQ 8 (fence F20). | 🤝 Aligned |  |
| R4.16 | v1 | P0 | The capture surface carries no cancel and no abandon control, and "End session" sits deliberately away from anything that advances the queue — the Dynamics 365 precedent of removing the abandon affordance from the counting surface ([acq v2 §8 Q8.4](../briefs/acquisition-experience-research-results-v2.md#8-progress-orientation-and-session-completion)). A test can list what the capture surface offers and assert that no such control is among them ([R11.12](#11-demo-device-and-verifiability)). | 🤝 Aligned |  |
| R4.17 | v1 | P0 | Nothing on the host is touched between the samples of one set and the operator never types metadata between scans ([acq v1 §1](../briefs/acquisition-experience-research-results.md)); the only typing in a bulk session is the operator's own "Add a swatch" ([§9](#9-ad-hoc-capture-and-one-row-sessions), fence F13). An announcement policy for assistive technology in a roughly three-second loop is OQ 14, and the capture state is announced without requiring focus. A test can see every announcement the app made ([R11.14](#11-demo-device-and-verifiability)), which is what lets a policy be judged rather than described. | ⌛️ Ready for Alignment — edited in round 6, re-review |  |
| R4.18 | v1 | P0 | A measurement already in flight when any queue action fires — "Flag", "Skip", "Pause", a jump, or "Add a swatch" — belongs to the row that was current at the trigger press the app accepted. "Re-take sample" and "Restart item" are not in that list: pressed while a scan is in flight they are rejected outright and discard nothing, so no reading ever changes hands through them ([R4.21](#4-the-scan-loop)). If that row no longer takes samples by the time the reading arrives, the reading is discarded and recorded as an attempt in that row's own history; it is never applied to another row (fence F32). A test can hold a reading open, fire each of those actions in turn, let the reading arrive, and read back that it landed on the row current at the press or went into that row's history — never onto the new current row ([R11.5](#11-demo-device-and-verifiability), [R11.11](#11-demo-device-and-verifiability)). | ⌛️ Ready for Alignment |  |
...
| R4.21 | v1 | P0 | Either action pressed while a scan is in flight is rejected with the rejection cue of [R4.3](#4-the-scan-loop) and discards nothing. Neither is a failure: neither counts toward K_FAILED_ATTEMPTS nor toward either of the guard's counters ([§5](#5-per-scan-failure-and-the-consecutive-failure-guard)). Until the operator presses the trigger on the new current row — the same boundary "Flag" uses (fence F16, [R5.6](#5-per-scan-failure-and-the-consecutive-failure-guard)) — both actions target the row that just landed rather than the new one; that row is captured and confirmed, so both answer with [E36](#12-error--state-copy) naming that row, because the correction there is a re-scan ([§8](#8-deferred-row-review-and-corrections)). After that press they act on the current row's own set, and either pressed on any other already-captured row answers with [E36](#12-error--state-copy) the same way. A test can take two of three samples, press "Re-take sample", and read back one sample kept, no attempt recorded, and the row still current ([R11.11](#11-demo-device-and-verifiability)). | ⌛️ Ready for Alignment |  |
...
| ID | Release | Pri | Requirement | Status | Commit PR |
| :--- | :--- | :--- | :--- | :--- | :--- |
| R5.1 | v1 | P0 | The per-scan failure set this PRD owns is exactly ambient light leakage, out-of-range temperature, a set of samples that disagree, and the operator's own Flag. There is no per-scan timer (fence F7). | 🤝 Aligned |  |
| R5.2 | v1 | P0 | Calibration drift, a device that returns no reading at all, a battery below the operational threshold, a disconnect, and a failed save are device-level halts rather than per-scan failures (fence F7; [device PRD §5](prd-device-management.md#5-mid-session-device-failure)); a completed set whose save fails is held through the halt for "Try saving again". (Handoff to the device PRD.) | 🤝 Aligned |  |
| R5.3 | v1 | P0 | On a per-scan failure a caution reaches the operator through at least two senses — a warning tone that is never the success tone, a haptic where available, and the on-screen state naming the cause in plain language ([E15](#12-error--state-copy), [E16](#12-error--state-copy)) — and it pre-empts any success tone in flight. The row then holds: the failed sample is set aside, good samples already taken are kept, the counter stays where it was, and the next trigger press is the retry on the same row, so a reflexive re-press after the warning tone can never land a reading on the next row (fence F6; [acq v2 §4 Q4.1](../briefs/acquisition-experience-research-results-v2.md#4-mid-run-error-handling)). Moving on is always a deliberate queue action, never the trigger. | 🤝 Aligned |  |
| R5.4 | v1 | P0 | A failed attempt is one of exactly two things: one trigger press the instrument refused, or one completed set whose samples disagreed. A sample the instrument accepted is not a failed attempt and does not reset the row's count; neither "Re-take sample" nor "Restart item" is one ([R4.21](#4-the-scan-loop)); leaving the row and coming back does not clear it, and a re-scan started from the set-aside list or the collection begins a fresh count while the row's record keeps every earlier attempt ([R8.13](#8-deferred-row-review-and-corrections)). K_FAILED_ATTEMPTS is a named provisional constant (candidate 3 — OQ 3): after that many failed attempts on one row — counted across any accepted samples that fell in between, since an accepted sample is not a failed attempt and does not reset the count — the row is deferred with its cause and its good samples kept, a distinct moved-on cue — neither the success tone nor the warning tone — reaches the operator through two senses ([E17](#12-error--state-copy)), the deferred tally increments, and the queue advances exactly once, so the operator keeps physical momentum ([acq v1 §4](../briefs/acquisition-experience-research-results.md)). | ⌛️ Ready for Alignment |  |
| R5.5 | v1 | P0 | Skip after a failure, and Skip mid-set with good samples and no failure, both defer the row with its cause and its samples kept and advance exactly once. Skip before any attempt on a row is plain navigation and leaves the row pending ([§6](#6-queue-navigation-and-reordering)). | 🤝 Aligned |  |
| R5.6 | v1 | P0 | Until the operator presses the trigger on the new current row, Flag targets the row that just landed — the most recent of the RECENTS_STRIP_LENGTH rows on the recents strip ([R4.15](#4-the-scan-loop)) — and moves it from captured to deferred: its reading is kept in the row's version history rather than as its current value, the row has no value until re-scanned, and it waits in the review like any other deferred row. The boundary is the operator's own next action, not a timer, and only the operator's Flag ever demotes a captured reading (fences F9, F16). | 🤝 Aligned |  |
| R5.7 | v1 | P0 | After any queue advance that was not a row-success confirmation — the moved-on cue of [R5.4](#5-per-scan-failure-and-the-consecutive-failure-guard), a Skip, or a Flag as missing — and before any trigger press on the new current row, "Flag" changes nothing, and says which of two things is true of the row just left ([E20](#12-error--state-copy)'s two variants): it is already set aside and waiting, or — where the advance was a "Skip" before any attempt, which is plain navigation ([R5.5](#5-per-scan-failure-and-the-consecutive-failure-guard), [R6.4](#6-queue-navigation-and-reordering)) — it is still pending and the queue will come back to it. Either way a reflexive "that one was wrong" never marks the next row missing (fence F16). There is nothing to demote in that window, because no row landed. | ⌛️ Ready for Alignment — edited in round 6, re-review |  |
| R5.8 | v1 | P0 | Otherwise Flag targets the current row: with no samples yet it is deferred with the cause "flagged: missing or damaged"; mid-set it is deferred with its good samples kept; either way the queue advances and the next press scans the next pending row (fences F9, F16). | 🤝 Aligned |  |
| R5.9 | v1 | P0 | Both of the guard's counters count rows, never trigger presses (fence F37). N_CONSEC_HARD is the number of consecutive rows that each ended in a deferral the instrument caused — auto-deferred after K_FAILED_ATTEMPTS, or "Skip" after a failed attempt (candidate 2 — OQ 3); N_CONSEC_FLAGGED is the number of consecutive rows the instrument set aside by either of those routes or by a set that disagreed (candidate 4 — OQ 3). Repeated failures on one swatch are that swatch's problem and not the instrument's: the row auto-defers on its own count ([R5.4](#5-per-scan-failure-and-the-consecutive-failure-guard)) without pausing the session, so that path stays reachable with the guard enabled. Either counter reaching its number pauses the session with a caution through two senses naming the likely cause — placement, light, temperature, or the material itself — and offering "Check placement and resume" or "Recalibrate" ([E19](#12-error--state-copy); [device PRD §3](prd-device-management.md#3-calibration)). A row reaching captured resets both counters, so an instrument that is working again clears its own record. The two may prove to be one counter once there is data to tune them, and OQ 3 carries that. The guard is modelled on Westgard multirules with clinical numbers that must be re-tuned for a hand-placed instrument ([acq v2 §4 Q4.2](../briefs/acquisition-experience-research-results-v2.md#4-mid-run-error-handling)). | ⌛️ Ready for Alignment |  |
| R5.10 | v1 | P0 | Rows the operator set aside by choice — Flag as missing or damaged, Skip mid-set with no failure, Flag on a row that just landed — never count toward either counter and never break a run, because the guard watches the instrument rather than the operator's decisions (fence F18). Lamp or calibration drift is not counted here either (fence F8). A test can flag five rows as missing in a row with the guard enabled, observe no pause, and read back both counters untouched ([R11.11](#11-demo-device-and-verifiability)). | ⌛️ Ready for Alignment — edited in round 6, re-review |  |
| R5.11 | v1 | P0 | The guard's pause keeps the held row under the instrument with its samples intact — nothing is discarded — and a trigger press during it is not accepted and asks the instrument for nothing. Force-resume is a deliberate action, never the trigger; it resets the guard's two counters but not the held row's own count toward K_FAILED_ATTEMPTS, so a row that keeps failing still defers on its own, and the held row is still current afterwards with its samples ([acq v2 §4 Q4.2](../briefs/acquisition-experience-research-results-v2.md#4-mid-run-error-handling)). The guard applies in the deferred-row review exactly as it does in the queue ([§8](#8-deferred-row-review-and-corrections)). | 🤝 Aligned |  |
| R5.12 | v1 | P0 | The guard has an explicit setting — enabled, or record-only, in which it counts and records but never pauses the session. The default is record-only through the dogfood phase, so untuned numbers never pause a session and never poison the data being gathered, and enabled at v1 once OQ 3's data exists (fence F23). Tolerance and failure status are recorded across roughly the first ten dogfood sessions in record-only and the constants tuned from that data ([acq v2 §4 Q4.2](../briefs/acquisition-experience-research-results-v2.md#4-mid-run-error-handling)); how far the numbers move is the remainder of OQ 6, which depends on OQ 3 and which covers this setting and the agreement check's ([R4.10](#4-the-scan-loop)) together (fence F27). Which setting a build starts in is an input a test can set, so both the record-only and the enabled path are walked without hardware. A dogfood build is not a release for the no-unresolved-constant rule ([Legend](#legend), fence F23). | 🤝 Aligned |  |
...
| ID | Release | Pri | Requirement | Status | Commit PR |
| :--- | :--- | :--- | :--- | :--- | :--- |
| R6.1 | v1 | P0 | A keyboard queue-management action opens a find field on the capture surface; typing narrows across all rows, pending first, then deferred, then captured, matching the start of a Swatch Code, any part of a Swatch Name, and the Swatch Alternate Code and Swatch Alternate Name the same two ways — the alternate code from the start, the alternate name in any part — all compared by the one matching rule ([R2.7](#2-inventory-import), fences F21, F11), because a swatch is known by more than one name and that is what the alternates are for. A hit that matched only an alternate says which field matched ([E41](#12-error--state-copy)), so an unexpected hit explains itself. A test can find one row by its alternate code and observe the result naming the alternate code as the field that matched. | 🤝 Aligned |  |
| R6.2 | v1 | P0 | Selecting a row makes it the current item at sample 0 and the collection's remembered row; the row the operator left stays pending, and the queue order is unchanged. Where the jump dropped a partial set, the capture surface says how many samples were let go and on which swatch ([R7.1](#7-pause-end-interruption-and-resume), [E42](#12-error--state-copy)), so the count is never lost in the move. | ⌛️ Ready for Alignment — edited in round 6, re-review |  |
| R6.3 | v1 | P0 | A hit on a captured row offers a re-scan rather than silently making it current ([E28](#12-error--state-copy), [§8](#8-deferred-row-review-and-corrections)); a hit on a deferred row becomes current and its re-scan resolves it; no match offers "Add a swatch" ([E37](#12-error--state-copy), [§9](#9-ad-hoc-capture-and-one-row-sessions)). | ⌛️ Ready for Alignment |  |
| R6.4 | v1 | P0 | Skip is navigation, not a state: it advances exactly once to the next pending row and leaves this one pending. No shipped system models "skipped" as a state ([acq v2 §1 Q1.1](../briefs/acquisition-experience-research-results-v2.md#1-queue-model-and-task-framing)). | 🤝 Aligned |  |
| R6.5 | v1 | P0 | The queue wraps to still-pending rows, so a row that was passed over is revisited without the operator tracking it. A pass begins at a position in the queue order — the place the current row occupied when the previous wrap-check ended, or when the session started — and ends when the queue comes back around to that position. The anchor is the position and not the row, so a row that stops being pending, is set aside, or is moved by a reorder cannot strand a pass; when that happens the anchor moves to the next pending position in queue order. A jump does not start a new pass, and a row the operator jumped away from without an attempt counts as skipped. | 🤝 Aligned |  |
| R6.6 | v1 | P0 | A wrap that finds only rows skipped without an attempt in this pass offers "Flag remaining as missing" — they become deferred with the cause "flagged: missing or damaged" and the review opens with them — or "End session", which leaves them pending for another day ([E21](#12-error--state-copy)). A skipped row can never make "complete" unreachable and the queue never circles forever. | 🤝 Aligned |  |
...
| R6.8 | v1 | P1 | The operator reorders before a session from the collection, or during a session from a queue list opened by a keyboard queue-management action (fence F5), either by dragging a pending row to a new position or by sorting on any metadata column — Swatch Code, Swatch Name, the alternates, or any other imported column. (Inherited obligation for the Collection Mode PRD: the sort and drag controls on the collection view; this PRD owns what a reorder does to the queue — fence F22.) | 🤝 Aligned |  |
| R6.9 | v1 | P1 | Sorts respect the user's language and read the numbers inside a code as numbers, so "A2" comes before "A10" and not after it; a code with no number sorts by its letters, so "A" comes before "A2"; and rows that tie keep the order they were already in, so sorting a hand-made order twice never shuffles the ties. A second application of the same sort toggles its direction. A sort applied while a manual order exists asks before replacing it ([E34](#12-error--state-copy)). (Inherited obligation for the Data Foundation PRD: comparing codes this way wherever they are ordered.) | 🤝 Aligned |  |
| R6.10 | v1 | P1 | A reorder never changes any row's state: captured rows are never re-queued and deferred rows stay deferred. Dragging a captured or deferred row is refused, and the app offers a re-scan or the review instead ([E33](#12-error--state-copy)). | 🤝 Aligned |  |
| R6.11 | v1 | P1 | Opening the queue list holds the current item rather than advancing it and keeps its samples, and a reorder mid-session does not discard them ([R7.1](#7-pause-end-interruption-and-resume)). The new order is saved with the collection, so a resumed session keeps it and the queue wraps in it; a re-import appends new rows after a manual order ([R2.11](#2-inventory-import)). | 🤝 Aligned |  |
| R6.12 | v1 | P1 | Three reordering specifics are open: REORDER_SCOPE — whether applying a sort repositions rows that are already captured, or leaves them where they are and orders only the pending ones — whether a sort is remembered as the collection's default order, and one-handed drag ergonomics with the instrument in the other hand (OQ 7). Until OQ 7 closes, a reorder moves pending rows only and leaves captured and deferred rows where they are ([R6.10](#6-queue-navigation-and-reordering)), so no build waits on the answer. Under Reading A of the seam a mid-session queue list is a second list inside the takeover; under Reading B it is the collection's own list (OQ 8, [§10](#10-the-seam-capture-to-collection)). | 🤝 Aligned |  |
...
| ID | Release | Pri | Requirement | Status | Commit PR |
| :--- | :--- | :--- | :--- | :--- | :--- |
| R7.1 | v1 | P0 | One partial-set rule, everywhere: the samples taken so far on the current row survive only a reorder and looking at the queue list. Any action that would put a different item under the instrument first — the operator's "Pause", a jump, entering the deferred-row review, "Add a swatch", a mid-session re-scan entered by find — and any device halt discards them; the row returns to sample 0 and stays pending. The rule has no exception for looking: opening the set-aside list mid-session discards the partial set whether or not the operator scans anything there, because the review puts a different swatch under the instrument (fence F19). The operator is never left to guess that it happened: at the moment the samples are dropped a distinct two-sense discard cue fires — not the success, caution, rejection, or moved-on cue — and the state the operator lands in says how many samples were dropped. Where the operator lands on a surface rather than in a state of its own — a jump, entering the review, an "Add a swatch" that went through — the capture surface carries a "⟨dropped⟩ samples on ⟨code⟩ let go" line naming the count and the swatch they came off ([E42](#12-error--state-copy)), and the set-aside list says the same in its header on review entry, so no discard is ever silent and a test can read the line off either surface ([R11.12](#11-demo-device-and-verifiability)). There is no confirmation to dismiss, because a prompt on every navigation would cost the heads-down loop a keypress it should never need (fence F26). A test can take one sample of a three-sample row, open the review, close it, observe the discard cue, and read back the row at sample 0 and still pending ([R11.7](#11-demo-device-and-verifiability), [R11.11](#11-demo-device-and-verifiability)). | ⌛️ Ready for Alignment |  |
...
| R7.9 | v1 | P0 | If the store file holding the user's collections has moved or is unavailable, the collection-unavailable state names it and the user finds the file ([E26](#12-error--state-copy)); nothing about the session is kept anywhere else. | 🤝 Aligned |  |
| R7.10 | v1 | P0 | A device halt left open at termination is closed as "unresolved — app terminated", so the user sees the interrupted session rather than a stale halt ([device PRD §5](prd-device-management.md#5-mid-session-device-failure)). (Handoff to the device PRD.) | 🤝 Aligned |  |
| R7.11 | v1 | P0 | An interruption that came when nothing was pending and no set-aside row was left unsettled ([R3.9](#3-the-capture-session), fence F31 as clarified) leaves nothing to resume: the interrupted session is closed as complete from the collection on relaunch, with no pre-flight gate and no "Resume capture", and the session summary appears as a state on the collection surface ([R7.15](#7-pause-end-interruption-and-resume), fence F20; placement follows OQ 8). | ⌛️ Ready for Alignment — edited in round 6, re-review |  |
| R7.12 | v1 | P0 | "Resume capture" starts a new session on the collection at the remembered row: the full pre-flight gate runs, including authorization, and whichever instrument is connected is bound, because quitting or crashing released the earlier one (fences F12, F14). A different instrument is used and the change is recorded in the session summary. Because the resume is a new session start, the device PRD's promises that a session runs to completion and is never interrupted for calibration apply to the new session only and do not carry across the break ([device PRD §2](prd-device-management.md#2-licensing--pre-authorization), [§3](prd-device-management.md#3-calibration)); an authorization window or a calibration that fell due while the app was gone is met at the gate before capture resumes. The interrupted session is closed as "interrupted — resumed by the next session" and never becomes active again, and the new session inherits its tallies and elapsed capture time so the summary reads as one run — a day with two crashes leaves three linked sessions. (Inherited obligation for the Data Foundation PRD.) | 🤝 Aligned |  |
| R7.13 | v1 | P0 | A resume with nothing pending and deferred rows waiting opens straight into the review ([R3.9](#3-the-capture-session)). An interrupted session can instead be ended from the collection without opening capture, its rows handled per R7.5. A one-row re-scan or ad-hoc add runs beside an unresumed interrupted bulk session, leaves it untouched, and never puts the operator back into the bulk queue (fence F17). | 🤝 Aligned |  |
| R7.14 | v1 | P0 | Three notes the device PRD must adopt: its "current item" — the first row with no saved reading ([device PRD §5](prd-device-management.md#5-mid-session-device-failure)) — is this PRD's remembered row whenever nothing was jumped or reordered; system sleep is a §5 halt for an active session rather than an interruption; and a resume after an interruption is a new session start, so the runs-to-completion and never-interrupted-for-calibration carve-outs are re-established rather than carried across it ([R7.12](#7-pause-end-interruption-and-resume)) (fences F12, F14, F15). (Inherited note for the device PRD; OQ 16.) | 🤝 Aligned |  |
| R7.15 | v1 | P0 | A session that completes ends in a summary that states the captured, deferred, and pending counts; the elapsed capture time ([R3.1](#3-the-capture-session)); the items-per-hour that follows from the two; the rows deliberately left set aside, named only when there are any; and the way into the collection ([E24](#12-error--state-copy)). It is a state on the collection surface rather than a dialog to dismiss, so the numbers stay readable and the state of the work is visible without opening capture (fence F20; placement follows OQ 8). A test can complete a session with and without set-aside rows and observe the summary state with the deferred line present in the first case and absent in the second ([R11.12](#11-demo-device-and-verifiability)). | ⌛️ Ready for Alignment |  |
...
| ID | Release | Pri | Requirement | Status | Commit PR |
| :--- | :--- | :--- | :--- | :--- | :--- |
| R8.1 | v1 | P0 | The set-aside list is entered four ways: the queue is exhausted with rows set aside, the operator chooses "Review the set-aside swatches" at any point, a session starts on a collection with nothing pending and an unsettled set-aside row waiting ([R3.9](#3-the-capture-session)), or a resume lands on a remembered row that was selected there ([R3.8](#3-the-capture-session)). Entering it discards any partial set on the current row, with the discard cue and, in the list's header, the count and the swatch it came off ([R7.1](#7-pause-end-interruption-and-resume), [E42](#12-error--state-copy), fences F19, F26). Whether the list is the same surface as the recents strip, adjacent to it, or separate is settled with the seam and is part of OQ 8 (fence F20, [§10](#10-the-seam-capture-to-collection)). | ⌛️ Ready for Alignment |  |
| R8.2 | v1 | P0 | The review lists every deferred row with its Swatch Code, Swatch Name, cause — light leak, temperature, samples disagreed, skipped mid-set, flagged as missing or damaged, flagged after capture — how many attempts were made, and how many samples it kept. | 🤝 Aligned |  |
| R8.3 | v1 | P0 | Selecting a deferred row makes it the current item and the collection's remembered row, marked as review-selected (fence F15), and a full set of N samples is then taken exactly as [§4](#4-the-scan-loop) describes. The prior failed samples are kept as history and never shown as the expected answer — blind re-capture, so an earlier reading cannot bias the operator ([acq v2 §11 Q11.4](../briefs/acquisition-experience-research-results-v2.md#11-the-seam--capture-to-collection)); a test lists what the surface shows and asserts the earlier value is not among it ([R11.12](#11-demo-device-and-verifiability)). | ⌛️ Ready for Alignment |  |
...
| R8.5 | v1 | P0 | "Leave it set aside" takes an optional note — the swatch is missing, damaged, or not worth the time today — and the row stays set aside with the decision and its note kept ([E27](#12-error--state-copy)). The decision settles the row: a settled row is adjudicated, so it counts toward completion ([R8.6](#8-deferred-row-review-and-corrections)), never routes a later session back into the review ([R3.9](#3-the-capture-session)), and can still be re-scanned by choice from the collection or the set-aside list (fence F31 as clarified). (Inherited obligation for the Data Foundation PRD.) | ⌛️ Ready for Alignment — edited in round 6, re-review |  |
| R8.6 | v1 | P0 | A session is complete only when every row is captured or has been deliberately left set aside after review, whether one row at a time ([R8.5](#8-deferred-row-review-and-corrections)) or all at once ([R8.15](#8-deferred-row-review-and-corrections)). Leaving the review before that ends the session early ([R7.5](#7-pause-end-interruption-and-resume)): the unresolved rows stay set aside, appear in the next session's list, and are marked in the collection. | ⌛️ Ready for Alignment |  |
| R8.15 | v1 | P0 | The review also offers one action that settles everything still outstanding: "Leave them all set aside" marks every remaining row as deliberately left, taking one optional note that applies to all of them, and satisfies [R8.6](#8-deferred-row-review-and-corrections)'s completion condition in a single keypress ([E39](#12-error--state-copy), fence F31). Every row it touches is settled on the same terms as [R8.5](#8-deferred-row-review-and-corrections)'s one-at-a-time decision: adjudicated, not reopened by a later session, still re-scannable by choice (fence F31 as clarified). Per-row "Leave it set aside" with its own note remains, so a hundred rows flagged missing never need a hundred decisions while a single interesting one can still carry its own reason. Each row keeps its own record of the decision. (Inherited obligation for the Data Foundation PRD.) | ⌛️ Ready for Alignment |  |
...
| ID | Release | Pri | Requirement | Status | Commit PR |
| :--- | :--- | :--- | :--- | :--- | :--- |
| R8.7 | v1 | P1 | A re-scan is entered by identifier — from the capture surface mid-session, or from an item in the collection with no session open — and never by a separate "re-scan this row" navigation ([acq v2 §11 Q11.4](../briefs/acquisition-experience-research-results-v2.md#11-the-seam--capture-to-collection)). The app says the row is captured and offers "Re-scan" ([E28](#12-error--state-copy)), and does not show the row's current value while re-scanning; a test lists what the surface shows and asserts the value is not among it ([R11.12](#11-demo-device-and-verifiability)). | ⌛️ Ready for Alignment |  |
...
| R8.10 | v1 | P1 | A re-scan whose samples disagree shows [E18](#12-error--state-copy)'s re-scan variant, which says the existing reading stays as it is unless a new one is accepted, and offers "Take it again", "Accept the average" — the new set becomes canonical with its spread recorded and the prior reading kept as history (fence F10) — or "Abandon the re-scan", which leaves the existing canonical value untouched and keeps the attempt in the row's history. Setting the row aside is not offered here, because the row is captured and nothing about it is waiting. | ⌛️ Ready for Alignment |  |
| R8.11 | v1 | P1 | QC and correction are offered explicitly rather than guessed, because a QC scan never touches the canonical value and a re-scan does ([E29](#12-error--state-copy); vision [J3](vision.md#j3-the-qc-pass-re-checker)). (Handoff to the QC & Comparison PRD.) | 🤝 Aligned |  |
...
| R8.13 | v1 | P0 | Version history keeps, for good, every reading the app kept — every sample of a saved set, and the samples kept on a row that became set aside with its record. Samples the operator dropped before a set was saved are not readings the app kept ([R7.1](#7-pause-end-interruption-and-resume), [R4.19](#4-the-scan-loop), [R4.20](#4-the-scan-loop), fence F26), and the promise does not stretch to cover them. It also keeps every attempt with its cause and its outcome, including failed re-scan attempts on a captured row; the operator's review decisions with their notes, whether the decision settled the row ([R8.5](#8-deferred-row-review-and-corrections), [R8.15](#8-deferred-row-review-and-corrections), fence F31 as clarified), and the recorded sample spread of an accepted average; a flagged reading moved out of the canonical position; the chain of sessions and which one resumed which; each row's queue order; and the remembered row with its review-selected mark and the three conditions that clear it. Every sample, attempt, guard pause, discard, and review decision records the session it belongs to and when it happened, so a rate or a share can be worked out per session and per session chain long after the run rather than only while it is on screen ([M4](#success-metrics), [M5](#success-metrics), [M9](#success-metrics)). A test reads all of it back after any run ([R11.11](#11-demo-device-and-verifiability)). (Inherited obligation for the Data Foundation PRD.) | ⌛️ Ready for Alignment |  |
| R8.14 | v1 | P1 | The simulated-readings banner and the recorded sample spread are surfaced where the collection is browsed. (Inherited obligation for the Collection Mode PRD.) | 🤝 Aligned |  |
...
| ID | Release | Pri | Requirement | Status | Commit PR |
| :--- | :--- | :--- | :--- | :--- | :--- |
| R9.1 | v1 | P1 | Adding an item takes Swatch Code — required — Swatch Name, and optionally Swatch Alternate Code and Swatch Alternate Name. A blank code is refused with the code-required state ([E31](#12-error--state-copy)) and the item cannot be saved. | 🤝 Aligned |  |
...
| R9.3 | v1 | P1 | Saving creates a pending row like any other. Metadata saved without a scan leaves the row pending and it joins the next bulk session's queue; nothing is lost. | 🤝 Aligned |  |
| R9.4 | v1 | P1 | With no session open on the collection, the scan runs as a one-row session: the pre-flight gate runs as it does for any capture ([device PRD UJ2](prd-device-management.md#uj-2-start-acquisition)), the connected instrument is bound, and the session ends itself when the row is captured or deferred, when the attempt is abandoned, or when the operator ends it from a halt or otherwise — so every device halt has a session to belong to and no one-row session is left open to block the next bulk session (fences F12, F17). It never moves the collection's remembered row ([R3.11](#3-the-capture-session)). A capture made while this collection's bulk session is paused is not this case ([R9.6](#9-ad-hoc-capture-and-one-row-sessions)). | 🤝 Aligned |  |
| R9.5 | v1 | P1 | A one-row session is not allowed while a session on a different collection is active or paused, and the app names the collection whose session holds the instrument ([E3](#12-error--state-copy)). | 🤝 Aligned |  |
| R9.6 | v1 | P1 | A capture on a collection whose bulk session is paused is a row captured inside that session, not a session of its own ([R3.11](#3-the-capture-session)): no second session record comes into being, the session keeps the instrument it already had, the pause is lifted for that one row, the pre-flight gate stays silent because its checks already passed and still pass ([R3.10](#3-the-capture-session)), and afterwards the queue is back on the row it left, still paused until the operator resumes it ([R7.3](#7-pause-end-interruption-and-resume)). The row counts in that session's tallies. | 🤝 Aligned |  |
| R9.7 | v1 | P1 | A one-row session runs on its own beside an unresumed interrupted bulk session and never wakes it; that session still waits for a deliberate "Resume capture", so a quick correction never puts the operator back into the bulk queue (fence F17). | 🤝 Aligned |  |
| R9.8 | v1 | P1 | An ad-hoc item whose samples exhaust K_FAILED_ATTEMPTS, or which the operator skips, is deferred exactly as a queue row would be, appears in the collection marked deferred, and is resolved by a re-scan later ([§8](#8-deferred-row-review-and-corrections)). | 🤝 Aligned |  |
| R9.9 | v1 | P1 | There is no flag-after-landing window on an item captured in a one-row session, because that session has already ended by the time the row lands ([R9.4](#9-ad-hoc-capture-and-one-row-sessions)): "that one was wrong" is a re-scan or a "Flag" from the collection, and either way the reading is kept as history (fence F9). A capture made on a paused bulk session is a row inside that session rather than a one-row session ([R9.6](#9-ad-hoc-capture-and-one-row-sessions)), so it keeps [R5.6](#5-per-scan-failure-and-the-consecutive-failure-guard)'s flag-after-landing window like any other row in it. | ⌛️ Ready for Alignment |  |
| R9.10 | v1 | P1 | Mid-session "Add a swatch" is a keyboard queue-management action that holds the current row rather than advancing it and discards any partial set on it ([R7.1](#7-pause-end-interruption-and-resume)); an add the operator saw through says on the capture surface how many samples were let go and off which swatch ([E42](#12-error--state-copy)), and cancelling the add returns the queue to the held row, creates nothing, and says the same ([E32](#12-error--state-copy)). Typing here is the operator's choice, never demanded by the queue (fence F13). | ⌛️ Ready for Alignment |  |
...
| ID | Release | Pri | Requirement | Status | Commit PR |
| :--- | :--- | :--- | :--- | :--- | :--- |
| R10.1 | v1 | P0 | Every reading lands in the real collection the moment it is saved; only the adjudication of deferred rows waits for session end, never the insertion of data. Deferred insertion at session end is never the design — its published failure is duplicate records on reconciliation ([acq v2 §11 Q11.2](../briefs/acquisition-experience-research-results-v2.md#11-the-seam--capture-to-collection)), and write-before-advance is already settled in the [device PRD §5](prd-device-management.md#5-mid-session-device-failure). True under either reading. | 🤝 Aligned |  |
| R10.2 | v1 | P0 | An always-visible recents strip of the last RECENTS_STRIP_LENGTH captured rows (candidate 5 — OQ 23) on the capture surface makes the just-captured row findable during the session ([R4.15](#4-the-scan-loop)). True under either reading. | 🤝 Aligned |  |
| R10.3 | v1 | P0 | The row states, the identifier vocabulary, and the counts are identical on the capture surface and on the collection surface — the data-model finding, which holds whichever navigation model the ADR picks ([acq v2 §11 Q11.6](../briefs/acquisition-experience-research-results-v2.md#11-the-seam--capture-to-collection)). | 🤝 Aligned |  |
| R10.4 | v1 | P0 | Capture shortcuts are inert whenever the capture surface does not have focus ([R4.4](#4-the-scan-loop)), and navigating within the app never ends a session. True under either reading; load-bearing under Reading B, where the same window serves both. | 🤝 Aligned |  |
| R10.5 | v1 | P0 | Mode indication depends on the reading: under Reading A the takeover is itself the indicator, and under Reading B at least two redundant, unmistakable indicators say the app is in capture ([acq v2 §11 Q11.6](../briefs/acquisition-experience-research-results-v2.md#11-the-seam--capture-to-collection)); a test can count the indicators the surface is showing rather than reading its wording ([R11.12](#11-demo-device-and-verifiability)). Gated on OQ 8. | 🤝 Aligned |  |
| R10.6 | v1 | P0 | The navigation fork — Reading A, capture as a modal takeover that hands off at session end, or Reading B, capture as a state of the live collection — is ADR-0004's and is settled by the owner's product call from a Demo Device prototype of each reading. The research default is Reading B, the only reading with shipped precedent (Lightroom Classic, Capture One, Dynamics 365), recorded and not decided. Gated on OQ 8. | 🤝 Aligned |  |
| R10.7 | v1 | P0 | The prototype's discriminating observables, all measurable on the Demo Device, all recorded the same way under both readings: the time and the number of steps to reach the just-captured row, at a pause and at session end; the time and the number of steps to reorder the queue mid-session and get back to the row that was held ([R6.11](#6-queue-navigation-and-reordering)); the count of mode slips; and where the set-aside list and the recents strip end up living, which is part of OQ 8 rather than a question of its own (fence F20). A mode slip is counted by a scripted task, not by instrumentation alone: an observer watches the operator work through a fixed script and counts the moments a capture action was intended and did not fire, cross-checked against the app's own record of every shortcut and control interaction, whoever delivered it, with the surface that had focus and the outcome ([R11.13](#11-demo-device-and-verifiability)); time and steps are taken from the same record. PROTOTYPE_PARTICIPANTS = 1: the owner walks the same script on both readings (fence F35), and walks it PROTOTYPE_RUNS times on each — a named provisional constant, candidate 3 (OQ 8) — alternating which reading goes first so that practice does not quietly favour one. Both numbers are stated with the result. The tie-break is pre-committed so the prototype cannot be read after the fact, and it asks the same of both readings: the reading with fewer mode slips totalled across all of its runs wins, and where those totals tie the reading with fewer steps to reach the just-captured row does. No single slip decides ADR-0004 (fence F38). Gated on OQ 8. | ⌛️ Ready for Alignment |  |
...
| ID | Release | Pri | Requirement | Status | Commit PR |
| :--- | :--- | :--- | :--- | :--- | :--- |
| R11.1 | v1 | P0 | Capture flows, states, and error surfaces are identical against the Demo Device and a live instrument, within the device PRD's closed list of exceptions as amended by the notes this PRD hands it ([R11.3](#11-demo-device-and-verifiability)), and CI exercises the simulated capture path on every PR ([device PRD §6](prd-device-management.md#6-mock-device-layer)). (Handoff to the device PRD.) | ⌛️ Ready for Alignment — edited in round 6, re-review |  |
| R11.2 | v1 | P0 | The capture surface carries an always-visible simulated indicator for the whole session, and every reading taken from the Demo Device is permanently marked simulated ([E35](#12-error--state-copy); [device PRD §6](prd-device-management.md#6-mock-device-layer)). (Handoff to the device PRD.) | 🤝 Aligned |  |
| R11.3 | v1 | P0 | The Demo Device's trigger is on-screen or keyboard, since there is no physical button, and whether a real instrument's button reaches the app at all is OQ 2. Where the trigger comes from is not on the device PRD's list of simulated-versus-live exceptions today, and it needs to be, since that list is closed and this is a real difference: the trigger source joins it, on the same footing as the transport, and falls away again if OQ 2 answers that the instrument's button never reaches the app. (Inherited note for the device PRD [§6](prd-device-management.md#6-mock-device-layer); OQ 16.) It paces at DEMO_SCAN_CYCLE — a named provisional constant standing for the real instrument's measured cycle, closed by the device PRD's OQ 10 (this document's OQ 22) — and never instantly, because an instant simulation hides every race the capture surface has. The configurable-latency row that provides that pacing is P1 in the device PRD and this PRD needs it in the first build phase, because the timing promises in [R4.8](#4-the-scan-loop) and [R4.13](#4-the-scan-loop) cannot be measured without it. (Inherited note for the device PRD [§6](prd-device-management.md#6-mock-device-layer); OQ 16.) | ⌛️ Ready for Alignment |  |
| R11.4 | v1 | P0 | Every failure this PRD relies on being able to produce on demand — ambient light leakage, out-of-range temperature, a save that fails, a disconnect, and an instrument that goes quiet — has a simulated equivalent under the device PRD's error-parity gate, where a live error with no simulated twin fails the build rather than review vigilance ([device PRD §6](prd-device-management.md#6-mock-device-layer)). (Handoff to the device PRD.) | 🤝 Aligned |  |
| R11.5 | v1 | P0 | A test can see every measurement the app asked the instrument for — when it started, how it ended, what asked for it, which row was current at the trigger press that was accepted, and which row the reading was finally saved to — so "exactly one measurement per accepted trigger", "a press during a lockout, a pause, or a halt asks the instrument for nothing" ([R4.2](#4-the-scan-loop)), and the in-flight ownership rule ([R4.18](#4-the-scan-loop)) are assertable rather than assumed. A test can also hold any single measurement open — slow, or never answering at all — for as long as it needs, so the lockout and the ownership rule can be exercised deliberately rather than by luck (fence F32). (Inherited obligation for the device PRD's simulated layer.) | 🤝 Aligned |  |
| R11.6 | v1 | P0 | A test can start from a declared device state — including an open halt record — rather than having to walk there ([§7](#7-pause-end-interruption-and-resume)). (Inherited obligation for the device PRD's simulated layer.) The matching collection-and-session starting state — rows in each state, an unresumed interrupted bulk session with the collection's remembered row, a review-selected mark, and a collection that breaks one of this document's own rules so the guards against it can be exercised ([R2.12](#2-inventory-import)) — belongs to what the app keeps, not to the instrument. (Inherited obligation for the Data Foundation PRD.) | 🤝 Aligned |  |
| R11.7 | v1 | P0 | The Demo Device's pacing and the windows that expire around it run on a clock a test can control. Every cue the operator can hear or feel is observable with its kind, whether it pre-empted a cue already in flight, its time on that controlled clock, and a second time taken from a clock the test cannot move; TRIGGER_ACK_WINDOW, RESULT_CUE_LATENCY, and ROW_CONFIRM_BUDGET ([R4.8](#4-the-scan-loop), [R4.13](#4-the-scan-loop)) are asserted against the second one, so moving the clock forward can never make a budget appear to be met. The cue set is closed — per-sample success, row-success, caution, rejection, moved-on, and discard — and no two of them sound or feel alike, which a test asserts pairwise. (Inherited obligation for the device PRD's simulated layer.) | 🤝 Aligned |  |
| R11.8 | v1 | P0 | A simulated reading carries the same set of per-mode measurements as a live one, including the collection's chosen scan mode ([R1.10](#1-collections), OQ 1), so the agreement check and every value worked out from a reading behave identically. (Inherited obligation for the device PRD's simulated layer.) | 🤝 Aligned |  |
| R11.9 | v1 | P0 | The contributor walk of [UJ3.9](#uj-39-capture-with-the-demo-device-contributor) is a verification sequence with a fixed order, because a row's own K_FAILED_ATTEMPTS is walked on one swatch and the guard is walked with a run of swatches (fence F37): the K_FAILED_ATTEMPTS path is walked with the guard in record-only, then the guard's pause with it enabled (OQ 6); then a row whose failures are interleaved with accepted samples, which must not pause the session while accepted samples intervene and must still auto-defer once its own failed attempts reach K_FAILED_ATTEMPTS ([R5.4](#5-per-scan-failure-and-the-consecutive-failure-guard), [R5.9](#5-per-scan-failure-and-the-consecutive-failure-guard)); then a pass whose anchor row is set aside part-way, which must still end rather than circle ([R6.5](#6-queue-navigation-and-reordering)); then a failed save and a disconnect, resume, and one captured row; then a quit and relaunch to "Resume capture"; then a force-quit during a halt, since quitting from a halt ends the session instead. After each, the captured / deferred / pending counts are exactly what they were, the current row is the collection's remembered row, and no row appears twice. The order follows from what each step needs — one swatch for a row's own count, a run of swatches for the guard's — and not from the candidate numbers, so no tuning of OQ 3 can invalidate it (fence F37). | ⌛️ Ready for Alignment |  |
| R11.10 | v1 | P0 | System sleep has no simulated equivalent; the stand-in for the sleep halt is a disconnect plus a change to one readiness input, then Resume ([device PRD §5](prd-device-management.md#5-mid-session-device-failure)). A test can also lose everything not yet safely written, at any moment it chooses, and take the drive away under the app, and then read back every row the operator was told landed — which is what makes the promise in [R4.13](#4-the-scan-loop) checkable rather than a claim a lazily-written build would also pass (fence F29). (Inherited obligation for the Data Foundation PRD's store.) A PR that touches device-facing capture code carries the `needs-hardware-verify` label and states in its body what needs verifying on real hardware ([AGENTS.md §5](../../AGENTS.md#5-hardware--the-public-repo-boundary)). | 🤝 Aligned |  |
| R11.11 | v1 | P0 | After any run, a test can read back every row's state, its canonical value, the samples kept on it, every attempt with its cause and its outcome, its version-history entries, the review decisions and notes on it and whether each settled the row ([R8.5](#8-deferred-row-review-and-corrections), fence F31 as clarified), the queue order, the remembered row with its review-selected mark, and the session chain with each session's status, tallies, instrument, and elapsed capture time. Each of those carries the session it belongs to and its time ([R8.13](#8-deferred-row-review-and-corrections)), so a per-session or per-chain rate is read back rather than reconstructed. Everything this document promises is kept is therefore checked against what is actually there rather than against what the screen said at the time. (Inherited obligation for the Data Foundation PRD.) | ⌛️ Ready for Alignment |  |
| R11.12 | v1 | P0 | A test can observe which named [§12](#12-error--state-copy) state the app is presenting, and, where a state is written in variants — [E2](#12-error--state-copy)'s empty and finished, [E18](#12-error--state-copy)'s in-the-queue and re-scan, [E20](#12-error--state-copy)'s already-set-aside and still-pending, [E33](#12-error--state-copy)'s scanned and set aside — which variant is up. A variant is not a state of its own: each state keeps one id however many variants it is written in. For whichever state is up, a test can also list which of its conditional lines and which of its actions are present — [E24](#12-error--state-copy)'s set-aside line and its review action, [E23](#12-error--state-copy)'s review-now action — so a line that appears only under some conditions is asserted rather than assumed ([R7.15](#7-pause-end-interruption-and-resume)). A test can list what each capture-owned surface offers and shows: the capture surface (its actions, the current row, the position counter, the sample counter, the tallies, the recents strip, the mode indicators, and the "⟨dropped⟩ samples on ⟨code⟩ let go" line — [R7.1](#7-pause-end-interruption-and-resume), [E42](#12-error--state-copy)); the queue list and the find field; the set-aside list, including every row it lists with its cause, its attempt count, and how many samples it kept ([R8.2](#8-deferred-row-review-and-corrections)) and its header's dropped-sample line on review entry ([R8.1](#8-deferred-row-review-and-corrections)); the re-scan surface, where the assertion that matters is that the row's existing value is not among what is shown ([R8.3](#8-deferred-row-review-and-corrections), [R8.7](#8-deferred-row-review-and-corrections)); the add-a-swatch form; and the session-summary state. The import preview is enumerated the same way — its three-way count of new, updated, and unchanged, its line for the rows the file does not mention, and its issue list ([R2.9](#2-inventory-import), [R2.13](#2-inventory-import), [R2.14](#2-inventory-import)). Presence, absence, which state is up, which variant of it, and which of its lines and actions are showing are therefore all assertable without matching any wording, so copy can change without breaking a behavioural test. | ⌛️ Ready for Alignment |  |
| R11.13 | v1 | P0 | A test can put focus on any surface in the app, deliver any capture shortcut, and observe whether it was accepted, rejected, or ignored — which is what makes "capture shortcuts are inert off the capture surface" ([R4.4](#4-the-scan-loop), [R10.4](#10-the-seam-capture-to-collection)) assertable. The app also records every capture shortcut and control interaction whoever delivered it — a test, or a person working the surface by hand — with the surface that had focus at the time and the accepted, rejected, or ignored outcome, and that record is readable after the run like everything else the app keeps ([R11.11](#11-demo-device-and-verifiability)). A human prototype run therefore leaves the same evidence a test does, which is what makes the mode-slip count of [R10.7](#10-the-seam-capture-to-collection) and the interactions-per-row of [M11](#success-metrics) measured rather than argued. | ⌛️ Ready for Alignment |  |
...
**Labels.** This table is the one place a user-facing label is written. A requirement row that names an action quotes the wording used here, and every action a row names appears in that state's copy below; where the two ever differ, this table is right. A few controls belong to no state of their own, so their wording is fixed here rather than below: "Start capture session", which begins a session on a collection; "Add a swatch", which adds one, from the collection or mid-session, and is the same wording the states below use for it; "Re-take sample"; "Restart item"; "Skip"; "Flag"; "Pause"; "Resume"; "End session"; and "Review the set-aside swatches".

**Placeholders.** ⟨…⟩ stands for something filled in when the state is shown. A sentence whose count would be zero is left out rather than shown as "0". A sentence carrying a count agrees with it — "1 swatch", "⟨n⟩ swatches". A named provisional constant renders as the number it currently holds, never as its name. ⟨code⟩ names the swatch the sentence is about. In a state that tells the operator where they will resume — [E22](#12-error--state-copy), [E23](#12-error--state-copy), [E25](#12-error--state-copy) — that is the row they will land on, so where the remembered row is no longer pending it is the row the fallback rule picks ([R3.7](#3-the-capture-session)); everywhere else it is whichever swatch the sentence names, which in [E17](#12-error--state-copy), [E20](#12-error--state-copy), [E36](#12-error--state-copy), and [E42](#12-error--state-copy) is the row just left rather than the new current one. ⟨dropped⟩ is the number of samples let go by the action that led here ([R7.1](#7-pause-end-interruption-and-resume), fence F26).
...
| E2 | Nothing to capture | Empty: "There's nothing to scan in this collection yet" · Finished: "You're done with ⟨collection⟩" | Empty: "Import your swatch list from a spreadsheet, or add one swatch at a time." · Finished: "Every swatch here is scanned or set aside for good. Add more swatches, or scan one again if something needs correcting." | Empty: Import a file; Add a swatch · Finished: Add a swatch; Re-scan a swatch | ⌛️ Ready for Alignment |
...
| E17 | Moved on | Moved on — ⟨code⟩ is waiting for you | That one didn't work after ⟨n⟩ tries, so it's set aside with the samples it did take. You'll come back to it at the end. Your next scan is the next swatch. | None (the queue has moved on) | ⌛️ Ready for Alignment |
| E18 | Samples disagree | These readings don't agree | In the queue: "The samples of this swatch came out further apart than expected. Take it again, accept the average of what you have, or set it aside for the end. Every reading is kept either way — nothing is averaged away without you." · Re-scan: "The samples of this swatch came out further apart than expected. Your existing reading stays as it is unless you accept a new one. Every reading is kept either way — nothing is averaged away without you." | In the queue: Take it again; Accept the average; Set it aside · Re-scan: Take it again; Accept the average; Abandon the re-scan | ⌛️ Ready for Alignment |
| E19 | Guard paused | Several swatches in a row haven't worked | ⟨n⟩ swatches in a row have gone wrong — the instrument either gave up on them or set them aside. It might be how the instrument is sitting, the light in the room, its temperature, or the swatches themselves if they're textured or metallic. Your swatch and its samples are still here. You can run a calibration without leaving this pause, and you come back and resume when you're ready. | Check placement and resume; Recalibrate | ⌛️ Ready for Alignment — edited in round 6, re-review |
| E20 | Flag has nothing to demote | Set aside: "⟨code⟩ is already set aside" · Still pending: "⟨code⟩ is still waiting in the queue" | Set aside: "The swatch you just left is waiting for you at the end. There's nothing here to flag." · Still pending: "You'll come back to it when the queue comes round. There's nothing here to flag." | None | ⌛️ Ready for Alignment |
...
| E23 | End early | Stop here for now? | What you've scanned stays scanned. Anything set aside stays set aside, and anything you haven't reached stays waiting. A part-finished swatch starts over next time. ⟨deferred⟩ swatches are set aside — you can deal with them now or another day. Your next session picks up at ⟨code⟩. | End session; Review them now — only while any are set aside; Keep scanning | 🤝 Aligned |
| E24 | Session complete | Done — ⟨captured⟩ swatches in ⟨elapsed⟩ | That's ⟨rate⟩ an hour. ⟨deferred⟩ swatches are set aside for good. Everything else is in your collection. *(Not copy: where this state lives follows the seam — fence F20, OQ 8.)* | Open the collection; Review the set-aside swatches — the second only while a set-aside swatch is still unsettled | ⌛️ Ready for Alignment |
| E25 | Resume capture | Pick up where you left off | ⟨captured⟩ scanned, ⟨deferred⟩ set aside, ⟨pending⟩ to go. Everything the app confirmed is safe. You'll start again at ⟨code⟩. | Resume capture; End that session | 🤝 Aligned |
...
| E27 | Leave one swatch set aside | Leave ⟨code⟩ set aside? | Add a note if you like — missing, damaged, or just not today. It stays in your collection, marked, with everything the instrument did read. | Leave it set aside; Try again | 🤝 Aligned |
| E28 | Re-scan offered | ⟨code⟩ already has a reading | Scanning it again makes the new reading its value and keeps the old one in its history. You won't see the old reading while you scan. The ⟨dropped⟩ samples you'd taken on the swatch you were on were let go to come here. | Re-scan; Cancel | 🤝 Aligned |
...
| E32 | Add a swatch cancelled | Nothing was added | You're back on ⟨code⟩. It starts again from its first sample; the ⟨dropped⟩ samples you'd taken on it were let go. | None | ⌛️ Ready for Alignment — edited in round 6, re-review |
...
| E39 | Leave every remaining swatch set aside | Leave the remaining ⟨n⟩ swatches set aside? | They stay in your collection, marked, with everything the instrument did read. Add one note for all of them if you like. This finishes the session. | Leave them all set aside; Go back | ⌛️ Ready for Alignment — edited in round 6, re-review |
...
| E41 | Matched on an alternate | Found ⟨name⟩ by its ⟨alternate code / alternate name⟩ | "⟨text⟩" isn't this swatch's own code or name — it's the alternate you gave it. Its code is ⟨code⟩. | Scan it; Keep looking | ⌛️ Ready for Alignment — edited in round 6, re-review |
| E42 | Samples let go | ⟨dropped⟩ samples on ⟨code⟩ let go | You'd started ⟨code⟩ before moving. It goes back to its first sample and is still waiting in the queue. | None (shown on the surface you land on) | ⌛️ Ready for Alignment |


Inherited obligations this PRD hands forward: the Data Foundation PRD owns what the app keeps, how nothing is ever destroyed, and how a test reads it all back ([R1.9](#1-collections), [R2.7](#2-inventory-import), [R2.10](#2-inventory-import), [R4.6](#4-the-scan-loop), [R4.11](#4-the-scan-loop), [R4.12](#4-the-scan-loop), [R4.13](#4-the-scan-loop), [R3.1](#3-the-capture-session), [R3.6](#3-the-capture-session), [R3.8](#3-the-capture-session), [R6.7](#6-queue-navigation-and-reordering), [R6.9](#6-queue-navigation-and-reordering), [R7.12](#7-pause-end-interruption-and-resume), [R8.5](#8-deferred-row-review-and-corrections), [R8.9](#8-deferred-row-review-and-corrections), [R8.13](#8-deferred-row-review-and-corrections), [R8.15](#8-deferred-row-review-and-corrections), [R11.6](#11-demo-device-and-verifiability), [R11.10](#11-demo-device-and-verifiability), [R11.11](#11-demo-device-and-verifiability)); the Collection Mode PRD owns rename, delete, renaming an imported column, the simulated-readings banner, and showing a recorded spread ([R1.8](#1-collections), [R2.6](#2-inventory-import), [R4.11](#4-the-scan-loop), [R8.14](#8-deferred-row-review-and-corrections)), and it also owns the sort and drag controls on the collection view that the operator uses to put the queue in order before a session, while this PRD keeps what a reorder does to the queue ([R6.8](#6-queue-navigation-and-reordering), fence F22); the QC & Comparison PRD owns the check-it path ([R8.11](#8-deferred-row-review-and-corrections)); the device PRD's simulated layer gains four obligations ([R11.5](#11-demo-device-and-verifiability), the device half of [R11.6](#11-demo-device-and-verifiability), [R11.7](#11-demo-device-and-verifiability), [R11.8](#11-demo-device-and-verifiability)), its §5 gains one note ([R7.14](#7-pause-end-interruption-and-resume)), and its §6 gains two more ([R11.3](#11-demo-device-and-verifiability)): the trigger source joins its closed exception list, gated on OQ 2, and its configurable-latency row moves from P1 into the first build phase. A third note for the device PRD is a matter of wording rather than behaviour: this document says "the instrument" where the device PRD's halt copy, which renders on the same capture surface, says "the device", and one noun should win when that document is next amended. What the Data Foundation PRD keeps includes the non-spectral mark on a reading averaged without the spectral entitlement ([R4.12](#4-the-scan-loop), fence F36). These are requirements on those documents, not suggestions. Device-path PRs still need a human with real hardware to verify before merge ([R11.10](#11-demo-device-and-verifiability)); these rows describe product behaviour only, and module layout is an open ADR ([AGENTS.md §3](../../AGENTS.md#3-decided--recommended--open)) that is not decided here.
...
| M11 | Interactions per captured row | Start: a row becoming current. End: that row reaching captured. Statistic: the median count of accepted interactions on that row that were not the trigger — every shortcut and control the operator used to get it captured. Population: every captured row on a dogfood run where that row itself had no failed attempt and was never set aside — cleanliness is judged per row, so one troublesome swatch does not take the whole run's rows out of the measurement. | Near zero (proposal) — the thesis is that a clean row costs N trigger presses and nothing else | The measurement record ([R11.5](#11-demo-device-and-verifiability)) and the record of shortcuts delivered and their outcome ([R11.13](#11-demo-device-and-verifiability)) | ⌛️ Ready for Alignment |
